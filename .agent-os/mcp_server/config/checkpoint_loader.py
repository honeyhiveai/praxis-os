...
class CheckpointLoader:
    ...
    def _load_with_fallback(
        ...
        if requirements:
            logger.info(
                "Loaded gate from YAML: %s:%s",
                workflow_type, phase
            )
            return requirements
        
        # Tier 2: RAG (not implemented in this task - returns None)
        requirements = self._load_from_rag(workflow_type, phase)  # pylint: disable=assignment-from-none
        if requirements:
            logger.info(
                "Loaded gate from RAG: %s:%s",
                workflow_type, phase
            )
            return requirements
        ...
        # Tier 3: Permissive
        logger.warning(
            "Using permissive gate (no YAML or RAG): %s:%s",
            workflow_type, phase
        )
