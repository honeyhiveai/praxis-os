...
class FrameworkGenerator:
    ...
    def _query_principles(self) -> Dict[str, str]:
        ...
        try:
            ...
            return {
                "three_tier": "Organize into Tier 1 (≤100 lines), Tier 2 (200-500), Tier 3 (outputs)",
...
"""
    ...
    def _generate_entry_point(
        ...
        commitments = [
            f"I will follow ALL {len(phases)} phases systematically (0-{len(phases)-1} in order)",
...
"""
    ...
    def _generate_phase_entry(
        ...
        is_output_phase = any(keyword in phase_name.lower() 
...
### Task 1: {phase_name}
🎯 NEXT-MANDATORY: [task-1-{phase_name.lower().replace(' ', '-')}.md](task-1-{phase_name.lower().replace(' ', '-')}.md)
...
"""
    ...
    def validate_compliance(self, framework: FrameworkDefinition) -> Dict[str, Any]:
        ...
        if len(framework.files) > 0:
            report["command_usage"] = int((files_with_commands / len(framework.files)) * 100)
            report["gate_coverage"] = int((files_with_gates / len(framework.files)) * 50)  # Expect ~50% to have gates
