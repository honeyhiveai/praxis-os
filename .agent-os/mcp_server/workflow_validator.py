...
class WorkflowValidator:
    ...
    def __init__(self, workflow_path: Path):
        ...
        :param workflow_path: Path to workflow directory (e.g., universal/workflows/my_workflow_v1)
        """
    ...
    def validate(self) -> Dict[str, Any]:
        ...
        compliance_score = 100 if total_checks == 0 else max(0, 100 - (len(self.issues) * 10 + len(self.warnings) * 3))
    ...
    def _validate_workflow_exists(self) -> None:
        ...
        if not self.workflow_path.exists():
            self.issues.append({
                ...
                "severity": "critical",
                "message": f"Workflow directory does not exist: {self.workflow_path}"
            })
        elif not self.workflow_path.is_dir():
            self.issues.append({
                "type": "not_directory",
                "severity": "critical",
                "message": f"Path exists but is not a directory: {self.workflow_path}"
            })
    ...
    def _validate_metadata_json(self) -> None:
        ...
        try:
            ...
            required_fields = ["workflow_type", "version", "description", "total_phases", 
                             "estimated_duration", "primary_outputs", "phases"]
            ...
            for field in required_fields:
                if field not in metadata:
                    self.issues.append({
                        ...
                        "path": "metadata.json",
                        "field": field,
                        "message": f"Required field '{field}' missing in metadata.json"
                    })
            ...
            if "phases" in metadata and "total_phases" in metadata:
                if len(metadata["phases"]) != metadata["total_phases"]:
                    self.issues.append({
                        ...
                        "message": f"total_phases ({metadata['total_phases']}) doesn't match phases array length ({len(metadata['phases'])})"
                    })
    ...
    def _validate_phase_structure(self) -> None:
        ...
        phase_dirs = [d for d in phases_dir.iterdir() if d.is_dir() and d.name.isdigit()]
        ...
        if not phase_dirs:
            self.issues.append({
                ...
                "severity": "error",
                "path": "phases/",
                "message": "No phase directories found (expected phases/0/, phases/1/, etc.)"
            })
            return
    ...
    def _validate_phase_directory(self, phase_dir: Path) -> None:
        ...
        if not phase_md.exists():
            ...
            if readme_md.exists():
                self.issues.append({
                    ...
                    "message": f"Phase {phase_num} uses README.md instead of phase.md (violates workflow-construction-standards.md)"
                })
            else:
                self.issues.append({
        ...
        if not task_files:
            self.warnings.append({
                ...
                "severity": "warning",
                "path": f"phases/{phase_num}/",
                "message": f"Phase {phase_num} has no task files (expected task-1-*.md, task-2-*.md, etc.)"
            })
    ...
    def _validate_file_naming(self) -> None:
        ...
        for monolithic_file in monolithic_files:
            self.issues.append({
                ...
                "message": f"Found monolithic phase file {monolithic_file.name}. Should use phases/N/ directories with phase.md instead.",
                "fix": "Split into phases/N/phase.md + phases/N/task-N-name.md files"
            })
            ...
            for task_file in phase_dir.glob("*.md"):
                ...
                if not task_file.name.startswith("task-"):
                    self.warnings.append({
                        ...
                        "message": f"Task file {task_file.name} doesn't follow task-N-name.md convention"
                    })
            ...
            if phase_md.exists():
                ...
                if line_count > self.PHASE_FILE_MAX:
                    self.warnings.append({
                        ...
                        "message": f"phase.md in Phase {phase_num} has {line_count} lines (target ~{self.PHASE_FILE_TARGET}, max {self.PHASE_FILE_MAX})",
                        "recommendation": "Consider splitting tasks into separate files or reducing overview content"
                    })
            ...
            for task_file in phase_dir.glob("task-*.md"):
                ...
                if line_count > self.TASK_FILE_ABSOLUTE_MAX:
                    ...
                    self.issues.append({
                        ...
                        "message": f"Task file {task_file.name} has {line_count} lines (absolute max {self.TASK_FILE_ABSOLUTE_MAX})",
                        "fix": "MUST split into multiple smaller task files (horizontal scaling required)"
                    })
                    ...
                    self.warnings.append({
                        ...
                        "message": f"Task file {task_file.name} has {line_count} lines (acceptable max {self.TASK_FILE_ACCEPTABLE_MAX}, absolute max {self.TASK_FILE_ABSOLUTE_MAX})",
                        "recommendation": f"Consider splitting if approaching {self.TASK_FILE_ABSOLUTE_MAX} lines"
                    })
                    ...
                    self.warnings.append({
                        ...
                        "message": f"Task file {task_file.name} has {line_count} lines (target <{self.TASK_FILE_TARGET_MAX}, acceptable up to {self.TASK_FILE_ACCEPTABLE_MAX})",
                        "note": "Acceptable size, but consider simplifying if possible"
                    })
    ...
    def print_report(self, report: Optional[Dict] = None) -> None:
        ...
        print(f"\n{'='*70}")
        print(f"Workflow Structure Validation Report")
        print(f"{'='*70}")
