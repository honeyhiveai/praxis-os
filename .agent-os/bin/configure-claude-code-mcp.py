...
Usage:
    python .agent-os/bin/configure-claude-code-mcp.py

The script will:
1. Read current MCP server port from .agent-os/.mcp_server_state.json
...
def find_project_root() -> Optional[Path]:
    """
    Find project root containing .agent-os directory.
    ...
    if (current / ".agent-os").exists():
        return current
    
    # Check parent directories (up to 5 levels)
    for parent in current.parents[:5]:
        if (parent / ".agent-os").exists():
            return parent
...
def main() -> int:
    ...
    print("\n📂 Searching for project root with .agent-os/...")
    project_root = find_project_root()
    
    if not project_root:
        print("❌ ERROR: Could not find .agent-os directory")
        print("\nMake sure:")
    ...
    try:
        ...
        print("  3. Check .agent-os/.mcp_server_state.json exists")
        return 1
