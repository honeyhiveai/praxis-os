...
# analysis logic. Refactoring would fragment cohesive parsing logic.
...
class SpecTasksParser(SourceParser):
    ...
    This is a critical component used by spec_execution_v1 workflow.
    """

    def parse(self, source_path: Path) -> List[DynamicPhase]:
        """
        Parse spec's tasks.md file using defensive semantic scoring.
        
        Implements phase shift detection for spec_execution_v1 workflow harness:
        - If Phase 0 found: applies +1 shift (Phase 0 becomes workflow Phase 1)
        - If Phase 1+ found: no shift needed
        - Validates sequential phases (no gaps allowed)
        ...
        # TODO: NEW ALGORITHM - Defensive parsing with semantic scoring (to be implemented)
        # For now, use existing AST-based extraction
    ...
    def _build_phase(  # pylint: disable=too-many-branches
        ...
        for _, node in enumerate(phase_data["nodes"]):
            ...
            # Extract tasks from markdown lists
                ...
                self._extract_tasks_from_list(node, phase_data, seen_validation_gate_header)
    ...
    def _extract_task_dependencies(self, text: str) -> List[str]:
        ...
        dep_text = self._extract_metadata(text, ["dependencies", "depends on", "requires", "after"])
        if not dep_text:
            return []
    ...
    def _parse_single_task(self, task_text: str) -> Optional[DynamicTask]:
        ...
        estimated_time = (
            self._extract_metadata(task_text, ["estimated time", "estimated duration", "time", "duration"])
            or "Variable"
        )
    ...
    def _flush_inline_buffer(self, inline_buffer: list, checkbox_marker: str, parts: list) -> str:
    ...
    def _extract_list_item_text(self, node: ListItem) -> str:
        ...
        for child in node.children if node.children else []:
            if isinstance(child, MarkdownList):
                ...
                checkbox_marker = self._flush_inline_buffer(inline_buffer, checkbox_marker, parts)
                inline_buffer = []
...
class WorkflowDefinitionParser(SourceParser):
    ...
    def _build_dynamic_task(self, task_data: dict, phase_number: int) -> Optional[DynamicTask]:
        """
    ...
    def _extract_validation_gate(self, validation_gate_data: dict) -> List[str]:
        ...
        for field_name, field_data in evidence_required.items():
            if isinstance(field_data, dict):
                ...
                criteria.append(f"{field_name} ({field_type}, {validator}): {description}")
            else:
