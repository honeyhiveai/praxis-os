...
- [ ] **Task 0.1**: Create component_helpers.py module (M - 2 hours)
  ...
  **Acceptance Criteria:**
  - [ ] File `component_helpers.py` exists at correct path
  - [ ] Module imports successfully: `from ouroboros.subsystems.rag.utils.component_helpers import *`
  - [ ] Module docstring explains fractal pattern (minimum 3 sentences)
  - [ ] Test file created and discovered by pytest

- [ ] **Task 0.2**: Implement ComponentDescriptor class (M - 2 hours)
  ...
  **Acceptance Criteria:**
  - [ ] ComponentDescriptor instantiates with all required fields
  - [ ] Empty name raises ValueError
  - [ ] Empty provides list raises ValueError
  - [ ] Empty capabilities list raises ValueError
  - [ ] Non-callable health_check raises ValueError
  - [ ] Non-callable rebuild raises ValueError
  - [ ] All 6 fields documented with examples in docstring

- [ ] **Task 0.3**: Implement dynamic_health_check() helper (M - 2 hours)
  ...
  **Acceptance Criteria:**
  - [ ] Function signature matches: `Dict[str, ComponentDescriptor] → HealthStatus`
  - [ ] Empty dict input returns HealthStatus(healthy=True)
  - [ ] Exception in component health_check() caught and logged
  - [ ] Result includes details["components"] dict
  - [ ] Result includes details["capabilities"] dict
  - [ ] Capability map correctly reflects component health (healthy=True, unhealthy=False)
  - [ ] Overall healthy=True only if ALL components healthy

- [ ] **Task 0.4**: Write unit tests for foundation (M - 3 hours)
  ...
  **Acceptance Criteria:**
  ...
  - [ ] Code coverage ≥ 90% for component_helpers.py
  - [ ] Test cases cover: valid descriptor, 5+ invalid descriptor variations
  - [ ] Test cases cover: 0 components, 1 component, 3 components
  - [ ] Test case: exception in component health_check() doesn't crash
  - [ ] Test case: all healthy → overall healthy
  - [ ] Test case: one broken → overall unhealthy
  - [ ] Test case: capabilities map correctly aggregated
...
- [ ] **Task 1.1**: Add components registry to GraphIndex.__init__() (S - 1 hour)
  ...
  **Acceptance Criteria:**
  - [ ] GraphIndex.__init__() creates self.components dict
  - [ ] self.components["ast"] exists and is ComponentDescriptor
  - [ ] self.components["graph"] exists and is ComponentDescriptor
  - [ ] AST component provides=["ast_nodes"], capabilities=["search_ast"]
  - [ ] Graph component provides=["symbols", "relationships"], capabilities=["find_callers", "find_dependencies", "find_call_paths"]
  - [ ] Both components have dependencies=[]
  - [ ] GraphIndex instantiates without errors

- [ ] **Task 1.2**: Implement _check_ast_health() method (M - 2 hours)
  ...
  **Acceptance Criteria:**
  - [ ] Method signature: `_check_ast_health(self) -> HealthStatus`
  - [ ] Returns HealthStatus (not Dict)
  - [ ] Details include: data_present (bool), query_works (bool), count (int), error (Optional[str])
  - [ ] When count > 0: healthy=True, data_present=True, query_works=True
  - [ ] When count = 0: healthy=False, data_present=False
  - [ ] Exception caught: returns HealthStatus with error field populated
  - [ ] No exceptions raised to caller

- [ ] **Task 1.3**: Implement _check_graph_health() method (M - 2 hours)
  ...
  **Acceptance Criteria:**
  - [ ] Method signature: `_check_graph_health(self) -> HealthStatus`
  - [ ] Returns HealthStatus with symbol_count and relationship_count in details
  - [ ] When counts > 0: healthy=True
  - [ ] When counts = 0: healthy=False
  - [ ] Exception caught and returned as error HealthStatus

- [ ] **Task 1.4**: Implement _rebuild_ast() method (M - 2 hours)
  - Clear only ast_nodes table: DELETE FROM ast_nodes
  - Log: "Cleared ast_nodes table"
  ...
  **Acceptance Criteria:**
  - [ ] Only ast_nodes table cleared (symbols/relationships preserved)
  - [ ] All source files re-parsed successfully
  - [ ] ast_nodes table count > 0 after rebuild
  - [ ] symbols/relationships tables unchanged (counts identical before/after)
  - [ ] Rebuild duration logged
  - [ ] File parse errors don't abort rebuild

- [ ] **Task 1.5**: Implement _rebuild_graph() method (M - 2 hours)
  - Clear symbols and relationships tables
  - Log: "Cleared symbols and relationships tables"
  ...
  **Acceptance Criteria:**
  - [ ] symbols and relationships tables cleared
  - [ ] ast_nodes table preserved (count unchanged)
  - [ ] symbols/relationships repopulated with counts > 0
  - [ ] Rebuild duration logged

- [ ] **Task 1.6**: Update health_check() to use dynamic_health_check() (S - 1 hour)
  - Replace existing health_check() body
  - Return: dynamic_health_check(self.components)
  - Remove static if/else logic
  - Add docstring: "Dynamic health check using component registry"
  - Verify: health_check() returns HealthStatus with components dict
  
  **Acceptance Criteria:**
  - [ ] health_check() body is single line: `return dynamic_health_check(self.components)`
  - [ ] No static if/else logic remains
  - [ ] Returns HealthStatus with details["components"] dict
  - [ ] Components dict has "ast" and "graph" entries
  - [ ] Docstring added

- [ ] **Task 1.7**: Write integration tests for partial degradation (L - 3 hours)
  ...
  **Acceptance Criteria:**
  ...
  - [ ] Test case: AST broken + graph healthy → find_callers() succeeds
  - [ ] Test case: AST broken → search_ast() fails with clear error message
  - [ ] Test case: Graph broken + AST healthy → search_ast() succeeds
  - [ ] Test case: _rebuild_ast() only → AST repopulated, graph data preserved
  - [ ] Rebuild time: < 3s (measured with time.perf_counter())
  - [ ] All 6+ test cases documented and passing
...
- [ ] GraphIndex has .components registry with 2 entries (ast, graph)
- [ ] Component-specific health check methods implemented
- [ ] Component-specific rebuild methods implemented
- [ ] health_check() delegates to dynamic_health_check()
- [ ] All integration tests passing
- [ ] Partial degradation test: AST broken → find_callers() succeeds ✅
- [ ] Targeted rebuild test: _rebuild_ast() < 3s ✅
- [ ] No linting errors
- [ ] Code reviewed and approved
...
- [ ] **Task 2.1**: Add components registry to CodeIndex.__init__() (M - 2 hours)
  ...
  **Acceptance Criteria:**
  - [ ] self.components dict created after sub-index initialization
  - [ ] Semantic component registered with lambda wrapper using default arg binding
  - [ ] Graph component registered with lambda wrapper using default arg binding
  - [ ] Lambda syntax: `lambda idx=self.semantic: idx.health_check()`
  - [ ] self.components has exactly 2 entries
  - [ ] CodeIndex instantiates without errors

- [ ] **Task 2.2**: Update health_check() to use dynamic_health_check() (S - 1 hour)
  - Replace existing health_check() body
  - Return: dynamic_health_check(self.components)
  ...
  **Acceptance Criteria:**
  - [ ] health_check() returns dynamic_health_check(self.components)
  - [ ] Returns HealthStatus with nested components dict
  - [ ] Components dict includes semantic and graph entries
  - [ ] Each component shows sub-component health (e.g., graph shows ast + graph)

- [ ] **Task 2.3**: Write integration tests for CodeIndex (M - 2 hours)
  - Test: Semantic broken, graph healthy → find_callers() succeeds
  ...
  **Acceptance Criteria:**
  - [ ] Test case: semantic broken + graph healthy → find_callers() succeeds
  - [ ] Test case: graph broken + semantic healthy → search_code() succeeds
  - [ ] Capabilities map correctly reflects health (find_callers available, search_ast unavailable)
  - [ ] Add mock component to GraphIndex: 0 lines changed in CodeIndex
  - [ ] All tests passing
...
- [ ] **Task 3.1**: Add components registry to StandardsIndex.__init__() (M - 3 hours)
  - Add self.components = {} dict
  - Register "vector" component (no dependencies)
  - Register "fts" component (dependencies=["vector"])
  - Register "reranker" component (dependencies=["vector", "fts"])
  - Implement _check_vector_health(), _check_fts_health(), _check_reranker_health()
  - Implement _rebuild_vector(), _rebuild_fts(), _rebuild_reranker()
  - Verify: self.components has 3 entries with correct dependency chain
  
  **Acceptance Criteria:**
  - [ ] 3 components registered (vector, fts, reranker)
  - [ ] Vector has dependencies=[]
  - [ ] FTS has dependencies=["vector"]
  - [ ] Reranker has dependencies=["vector", "fts"]
  - [ ] All 3 health check methods implemented
  - [ ] All 3 rebuild methods implemented

- [ ] **Task 3.2**: Update health_check() to use dynamic_health_check() (S - 1 hour)
  - Replace existing health_check() body
  - Return: dynamic_health_check(self.components)
  - Verify: Returns HealthStatus with vector + fts + reranker components
  
  **Acceptance Criteria:**
  - [ ] health_check() uses dynamic_health_check(self.components)
  - [ ] Returns HealthStatus with 3 component entries
  - [ ] Capabilities map includes vector_search, keyword_search, hybrid_search

- [ ] **Task 3.3**: Write integration tests for StandardsIndex (M - 2 hours)
  - Test: FTS broken, vector healthy → vector_search() succeeds
  - Test: Reranker broken, vector + FTS healthy → keyword_search() succeeds
  - Verify dependency validation (FTS cannot exist without vector)
  - Verify capabilities map
  
  **Acceptance Criteria:**
  - [ ] Test case: FTS broken + vector healthy → vector_search() succeeds
  - [ ] Test case: reranker broken + vector/FTS healthy → keyword_search() succeeds
  - [ ] Dependency validation tested (if implemented)
  - [ ] All tests passing
