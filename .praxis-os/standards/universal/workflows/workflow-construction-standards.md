...
|-----------|---------------|
| **Creating workflow** | `pos_search(content_type="standards", query="workflow construction")` |
| **Workflow structure** | `pos_search(content_type="standards", query="workflow structure")` |
| **Phase files** | `pos_search(content_type="standards", query="phase.md")` |
| **Task files** | `pos_search(content_type="standards", query="task file structure")` |
| **File sizes** | `pos_search(content_type="standards", query="workflow file sizes")` |
| **Validation gates** | `pos_search(content_type="standards", query="workflow validation gates")` |
| **Command language** | `pos_search(content_type="standards", query="workflow commands")` |
| **Building workflows** | `pos_search(content_type="standards", query="building workflows")` |
...
1. **Start with construction** → `pos_search(content_type="standards", query="workflow construction")` (this document)
2. **Add metadata** → `pos_search(content_type="standards", query="workflow metadata")` → `standards/workflows/workflow-metadata-standards.md`
3. **Understand system** → `pos_search(content_type="standards", query="workflow system overview")` → `standards/workflows/workflow-system-overview.md`
4. **Learn principles** → `pos_search(content_type="standards", query="framework creation principles")` → `standards/meta-workflow/framework-creation-principles.md`
5. **Apply architecture** → `pos_search(content_type="standards", query="three-tier architecture")` → `standards/meta-workflow/three-tier-architecture.md`
...
- `standards/workflows/workflow-system-overview.md` - Workflow engine → `pos_search(content_type="standards", query="workflow system overview")`
- `standards/workflows/mcp-rag-configuration.md` - RAG configuration → `pos_search(content_type="standards", query="MCP RAG configuration")`
...
- `standards/meta-workflow/horizontal-decomposition.md` - File size guidelines → `pos_search(content_type="standards", query="horizontal decomposition")`
