...
await pos_search(content_type="standards", query="How do I generate tests for Python?")
await pos_search(content_type="standards", query="What workflows create unit tests?")
await pos_search(content_type="standards", query="Automated test generation with validation")
...
|-----------|---------------|
| **Creating workflow** | `pos_search(content_type="standards", query="workflow metadata")` |
| **Required fields** | `pos_search(content_type="standards", query="workflow metadata schema")` |
| **File location** | `pos_search(content_type="standards", query="where workflow metadata")` |
| **Making discoverable** | `pos_search(content_type="standards", query="workflow discovery")` |
| **Quality standards** | `pos_search(content_type="standards", query="workflow metadata quality")` |
| **Validation** | `pos_search(content_type="standards", query="validate workflow metadata")` |
| **Naming conventions** | `pos_search(content_type="standards", query="workflow naming")` |
| **Searchability** | `pos_search(content_type="standards", query="searchable workflow descriptions")` |
...
1. **Start with metadata standards** → `pos_search(content_type="standards", query="workflow metadata")` (this document)
...
3. **Learn RAG indexing** → `pos_search(content_type="standards", query="MCP RAG configuration")` → `standards/workflows/mcp-rag-configuration.md`
4. **Learn construction standards** → `pos_search(content_type="standards", query="workflow construction")` → `standards/workflows/workflow-construction-standards.md`
...
**Meta-Framework:**
- `standards/meta-workflow/validation-gates.md` - Checkpoint criteria → `pos_search(content_type="standards", query="validation gates")`
