...
|-----------|---------------|
| **Slow code** | `pos_search(content_type="standards", query="performance optimization")` |
| **Profiling** | `pos_search(content_type="standards", query="how to profile code")` |
| **Bottlenecks** | `pos_search(content_type="standards", query="identify bottlenecks")` |
| **Caching** | `pos_search(content_type="standards", query="caching patterns")` |
| **Algorithm choice** | `pos_search(content_type="standards", query="algorithmic complexity")` |
| **Before optimizing** | `pos_search(content_type="standards", query="premature optimization")` |
| **Performance targets** | `pos_search(content_type="standards", query="performance targets")` |
| **Measurement** | `pos_search(content_type="standards", query="how to measure performance")` |
...
1. **Start with optimization patterns** → `pos_search(content_type="standards", query="performance optimization")` (this document)
2. **Learn database optimization** → `pos_search(content_type="standards", query="database patterns")` → `standards/database/database-patterns.md`
3. **Understand concurrency** → `pos_search(content_type="standards", query="concurrency patterns")` → `standards/concurrency/`
4. **Learn testing** → `pos_search(content_type="standards", query="performance testing")` → `standards/testing/integration-testing.md`

**By Category:**

**Database:**
- `standards/database/database-patterns.md` - Database optimization → `pos_search(content_type="standards", query="database patterns")`

**Concurrency:**
- `standards/concurrency/race-conditions.md` - Concurrent safety → `pos_search(content_type="standards", query="race conditions")`
- `standards/concurrency/locking-strategies.md` - Locking patterns → `pos_search(content_type="standards", query="locking strategies")`

**Testing:**
- `standards/testing/integration-testing.md` - Performance tests → `pos_search(content_type="standards", query="integration testing")`
- `standards/testing/property-based-testing.md` - Load testing → `pos_search(content_type="standards", query="property based testing")`

**AI Safety:**
- `standards/ai-safety/production-code-checklist.md` - Production requirements → `pos_search(content_type="standards", query="production code checklist")`

**Failure Modes:**
- `standards/failure-modes/timeout-patterns.md` - Timeout handling → `pos_search(content_type="standards", query="timeout patterns")`
- `standards/failure-modes/circuit-breakers.md` - Resilience → `pos_search(content_type="standards", query="circuit breakers")`
