...
- Planning integration tests → `pos_search(content_type="standards", query="integration testing patterns")`
- Database testing strategy → `pos_search(content_type="standards", query="test database strategies")`
- Testing external APIs → `pos_search(content_type="standards", query="external service testing")`
- Test data management → `pos_search(content_type="standards", query="test fixtures factories")`
- Slow integration tests → `pos_search(content_type="standards", query="fast integration tests")`
...
pos_search(content_type="standards", query="test pyramid unit integration e2e")

# For test doubles and mocking
pos_search(content_type="standards", query="test doubles mocks stubs spies")

# For database patterns
pos_search(content_type="standards", query="database patterns repository")

# For API design and testing
pos_search(content_type="standards", query="API design principles testing")
```
...
pos_search(content_type="standards", query="fast integration tests database")

# When managing test data
pos_search(content_type="standards", query="test fixtures factories builders")

# When testing external services
pos_search(content_type="standards", query="external service testing mocking")
```
...
pos_search(content_type="standards", query="pytest fixtures test client python")

# Java integration testing  
pos_search(content_type="standards", query="spring boot test testcontainers")

# JavaScript integration testing
pos_search(content_type="standards", query="supertest jest integration tests")
```
