...
1. **Testing Complex Algorithms**
   ...
   - Query: `pos_search(content_type="standards", query="property-based testing algorithms")`

2. **Finding Edge Cases**
   - Situation: Want to find bugs example-based tests miss
   - Query: `pos_search(content_type="standards", query="property-based testing edge cases")`

3. **Testing API Invariants**
   - Situation: API has properties that should always hold
   - Query: `pos_search(content_type="standards", query="what properties to test")`

4. **Learning Property-Based Testing**
   - Situation: Never used PBT before, want to understand
   - Query: `pos_search(content_type="standards", query="what is property-based testing")`

5. **Debugging Shrinking Issues**
   - Situation: Property test failing, need to understand shrinking
   - Query: `pos_search(content_type="standards", query="shrinking property testing")`

6. **Choosing Test Framework**
   - Situation: Want to add PBT to project
   - Query: `pos_search(content_type="standards", query="property-based testing frameworks")`

### Query by Use Case

| Use Case | Example Query |
|----------|---------------|
| Learn PBT | `pos_search(content_type="standards", query="what is property-based testing")` |
| Write properties | `pos_search(content_type="standards", query="types of properties PBT")` |
| Generate test data | `pos_search(content_type="standards", query="generators property testing")` |
| Debug failures | `pos_search(content_type="standards", query="shrinking property testing")` |
| Common patterns | `pos_search(content_type="standards", query="round-trip property testing")` |
| Choose framework | `pos_search(content_type="standards", query="Hypothesis vs QuickCheck")` |
...
- `standards/testing/test-pyramid.md` - Where PBT fits in test strategy
  → `pos_search(content_type="standards", query="test pyramid structure")`
- `standards/testing/test-doubles.md` - May need mocks for property tests
  → `pos_search(content_type="standards", query="how to use test doubles")`
- `standards/testing/integration-testing.md` - PBT complements integration tests
  → `pos_search(content_type="standards", query="integration testing patterns")`
...
1. **Learn Concept**: `pos_search(content_type="standards", query="what is property-based testing")` → Understand approach
2. **Identify Properties**: `pos_search(content_type="standards", query="types of properties")` → Find invariants, round-trips
3. **Choose Framework**: `pos_search(content_type="standards", query="property testing frameworks")` → Select Hypothesis/fast-check/jqwik
4. **Write Tests**: `pos_search(content_type="standards", query="generators property testing")` → Generate test data
5. **Debug**: `pos_search(content_type="standards", query="shrinking property testing")` → Understand minimal failures
6. **Refine**: `pos_search(content_type="standards", query="property testing best practices")` → Improve properties
