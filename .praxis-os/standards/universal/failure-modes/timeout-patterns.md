...
|-----------|---------------|
| **Network operations** | `pos_search(content_type="standards", query="timeout patterns")` |
| **API integration** | `pos_search(content_type="standards", query="request timeout")` |
| **Slow operations** | `pos_search(content_type="standards", query="connection timeout")` |
| **Timeout configuration** | `pos_search(content_type="standards", query="timeout configuration")` |
| **Hanging operations** | `pos_search(content_type="standards", query="timeout strategies")` |
| **Retry timeouts** | `pos_search(content_type="standards", query="timeout with retry")` |
| **Cascading failures** | `pos_search(content_type="standards", query="cascading timeouts")` |
...
1. **Set timeouts** → `pos_search(content_type="standards", query="timeout patterns")` (this document)
2. **Add retries** → `pos_search(content_type="standards", query="retry strategies")` → `standards/failure-modes/retry-strategies.md`
3. **Add circuit breaker** → `pos_search(content_type="standards", query="circuit breaker")` → `standards/failure-modes/circuit-breakers.md`
4. **Plan degradation** → `pos_search(content_type="standards", query="graceful degradation")` → `standards/failure-modes/graceful-degradation.md`

**By Category:**

**Failure Modes:**
- `standards/failure-modes/retry-strategies.md` - Retry logic (total timeout must account for retries) → `pos_search(content_type="standards", query="retry strategies")`
...
- `standards/failure-modes/graceful-degradation.md` - Fallback strategies for timeouts → `pos_search(content_type="standards", query="graceful degradation")`

**Testing:**
- `standards/testing/integration-testing.md` - Testing timeout behavior → `pos_search(content_type="standards", query="integration testing")`

**Database:**
- `standards/database/database-patterns.md` - Database timeout configuration → `pos_search(content_type="standards", query="database patterns")`

**AI Safety:**
- `standards/ai-safety/production-code-checklist.md` - Production code checklist (includes timeout validation) → `pos_search(content_type="standards", query="production code checklist")`
