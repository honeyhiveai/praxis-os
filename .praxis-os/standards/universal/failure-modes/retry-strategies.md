...
|-----------|---------------|
| **Network call failures** | `pos_search(content_type="standards", query="retry strategies")` |
| **API integration** | `pos_search(content_type="standards", query="exponential backoff")` |
| **Transient errors** | `pos_search(content_type="standards", query="when to retry failures")` |
| **Distributed systems** | `pos_search(content_type="standards", query="retry with jitter")` |
| **Duplicate operations concern** | `pos_search(content_type="standards", query="idempotency retries")` |
| **Retry tuning** | `pos_search(content_type="standards", query="retry decision matrix")` |
| **Thundering herd** | `pos_search(content_type="standards", query="jitter retry")` |
...
1. **Start here** → `pos_search(content_type="standards", query="retry strategies")`
2. **Then protect** → `pos_search(content_type="standards", query="circuit breaker")` → `standards/failure-modes/circuit-breakers.md`
3. **Then degrade** → `pos_search(content_type="standards", query="graceful degradation")` → `standards/failure-modes/graceful-degradation.md`
4. **Then timeout** → `pos_search(content_type="standards", query="timeout patterns")` → `standards/failure-modes/timeout-patterns.md`

**By Category:**

**Failure Modes:**
- `standards/failure-modes/circuit-breakers.md` - System-wide failure protection → `pos_search(content_type="standards", query="circuit breaker")`
- `standards/failure-modes/graceful-degradation.md` - Degrade functionality when services fail → `pos_search(content_type="standards", query="graceful degradation")`
...
**Testing:**
- `standards/testing/integration-testing.md` - Testing retry logic → `pos_search(content_type="standards", query="integration testing")`

**Database:**
- `standards/database/database-patterns.md` - Database retry strategies → `pos_search(content_type="standards", query="database retry")` 
