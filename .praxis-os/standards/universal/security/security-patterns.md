...
|-----------|---------------|
| **User input handling** | `pos_search(content_type="standards", query="input validation")` |
| **SQL queries** | `pos_search(content_type="standards", query="prevent SQL injection")` |
| **Password storage** | `pos_search(content_type="standards", query="hash passwords")` |
| **Authentication** | `pos_search(content_type="standards", query="secure authentication")` |
| **Authorization** | `pos_search(content_type="standards", query="authorization patterns")` |
| **Sensitive data** | `pos_search(content_type="standards", query="encryption")` |
| **XSS prevention** | `pos_search(content_type="standards", query="prevent XSS")` |
| **API security** | `pos_search(content_type="standards", query="secure API")` |
| **Security vulnerabilities** | `pos_search(content_type="standards", query="OWASP Top 10")` |
| **Security testing** | `pos_search(content_type="standards", query="security testing")` |
...
1. **Start here** → `pos_search(content_type="standards", query="security patterns")` (this document)
2. **Then validate inputs** → `pos_search(content_type="standards", query="input validation")`
3. **Then test** → `pos_search(content_type="standards", query="security testing")` (this document)
4. **Then review** → `pos_search(content_type="standards", query="production code checklist")` → `standards/ai-safety/production-code-checklist.md`

**By Category:**

**Testing:**
- `standards/testing/integration-testing.md` - Testing authentication and authorization → `pos_search(content_type="standards", query="integration testing")`
- `standards/testing/test-pyramid.md` - Security test coverage ratios → `pos_search(content_type="standards", query="test pyramid")`

**Architecture:**
- `standards/architecture/api-design-principles.md` - Secure API design → `pos_search(content_type="standards", query="API design")`
- `standards/architecture/dependency-injection.md` - Injecting security services → `pos_search(content_type="standards", query="dependency injection")`

**Database:**
- `standards/database/database-patterns.md` - Preventing SQL injection → `pos_search(content_type="standards", query="database patterns")`

**AI Safety:**
- `standards/ai-safety/production-code-checklist.md` - Production security checklist → `pos_search(content_type="standards", query="production code checklist")`

**Documentation:**
- `standards/documentation/api-documentation.md` - Documenting security requirements → `pos_search(content_type="standards", query="API documentation")`
