...
1. **Designing New Classes**
   ...
   - Query: `pos_search(content_type="standards", query="how to inject dependencies")`

2. **Making Code Testable**
   - Situation: Need to mock dependencies for unit tests
   - Query: `pos_search(content_type="standards", query="how to make code testable")`

3. **Refactoring Hard-Coded Dependencies**
   - Situation: Code has hard-coded database/API/service instantiation
   - Query: `pos_search(content_type="standards", query="how to remove hard-coded dependencies")`

4. **Choosing DI Implementation Pattern**
   - Situation: Deciding between manual DI, container, or factory
   - Query: `pos_search(content_type="standards", query="when to use DI container")`

5. **Resolving Circular Dependencies**
   - Situation: Two classes depend on each other
   - Query: `pos_search(content_type="standards", query="how to resolve circular dependencies")`

6. **Code Review Feedback**
   - Situation: Reviewer says "this should use dependency injection"
   - Query: `pos_search(content_type="standards", query="dependency injection pattern")`

### Query by Use Case

| Use Case | Example Query |
|----------|---------------|
| Make code testable | `pos_search(content_type="standards", query="how to make code testable")` |
| Remove hard-coding | `pos_search(content_type="standards", query="avoid hard-coded dependencies")` |
| Choose DI type | `pos_search(content_type="standards", query="constructor vs setter injection")` |
| Handle circular deps | `pos_search(content_type="standards", query="circular dependencies solution")` |
| DI container | `pos_search(content_type="standards", query="when to use DI container")` |
...
- `standards/architecture/solid-principles.md` - SOLID principles (DI supports Dependency Inversion)
  → `pos_search(content_type="standards", query="how to apply SOLID principles")`
- `standards/architecture/separation-of-concerns.md` - Separating concerns makes DI easier
  → `pos_search(content_type="standards", query="separation of concerns")`
...
1. **Learn Pattern**: `pos_search(content_type="standards", query="dependency injection pattern")` → Read this standard
2. **Learn Testing**: `pos_search(content_type="standards", query="how to use test doubles")` → Understand mocking
3. **Choose Type**: `pos_search(content_type="standards", query="constructor vs setter injection")` → Select DI type
4. **Implement**: Apply constructor injection to your classes
5. **Test**: `pos_search(content_type="standards", query="how to unit test with mocks")` → Write tests with mocked dependencies
6. **Review**: `pos_search(content_type="standards", query="dependency injection anti-patterns")` → Validate approach
