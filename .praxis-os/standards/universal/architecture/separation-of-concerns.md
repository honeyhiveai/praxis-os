...
1. **Designing New Features**
   ...
   - Query: `pos_search(content_type="standards", query="how to organize code by concern")`

2. **Refactoring Tangled Code**
   - Situation: Code has multiple responsibilities mixed together
   - Query: `pos_search(content_type="standards", query="separation of concerns pattern")`

3. **Choosing Architecture Pattern**
   - Situation: Deciding on MVC vs Repository vs Service Layer vs Hexagonal
   - Query: `pos_search(content_type="standards", query="what separation pattern to use")`
...
5. **Code Review Feedback**
   ...
   - Query: `pos_search(content_type="standards", query="separation of concerns violations")`

6. **Identifying God Objects**
   - Situation: Class doing too many things
   - Query: `pos_search(content_type="standards", query="God Object anti-pattern")`

### Query by Use Case

| Use Case | Example Query |
|----------|---------------|
| Organize code | `pos_search(content_type="standards", query="how to organize code by concern")` |
| Refactor tangled code | `pos_search(content_type="standards", query="separation of concerns refactoring")` |
| Choose pattern | `pos_search(content_type="standards", query="MVC vs repository pattern")` |
| Improve testability | `pos_search(content_type="standards", query="make code testable")` |
| Fix God Object | `pos_search(content_type="standards", query="God Object anti-pattern")` |
...
1. **Learn Principle**: `pos_search(content_type="standards", query="separation of concerns")` → Read this standard
2. **Learn SOLID**: `pos_search(content_type="standards", query="single responsibility principle")` → Understand SRP
3. **Choose Pattern**: `pos_search(content_type="standards", query="MVC vs repository pattern")` → Select approach
4. **Implement**: Apply chosen pattern to your code
5. **Test**: `pos_search(content_type="standards", query="how to test with mocks")` → Write tests for each concern
6. **Review**: `pos_search(content_type="standards", query="separation of concerns violations")` → Validate approach
