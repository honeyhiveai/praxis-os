...
1. **Starting New API Design**
   ...
   - Query: `pos_search(content_type="standards", query="how to design APIs")`

2. **During API Review**
   - Situation: Reviewing proposed API changes
   - Query: `pos_search(content_type="standards", query="API design best practices")`

3. **Debugging API Usability Issues**
   - Situation: Users find your API confusing or error-prone
   - Query: `pos_search(content_type="standards", query="API usability principles")`

4. **Choosing HTTP Status Codes**
   - Situation: Not sure which HTTP status code to return
   - Query: `pos_search(content_type="standards", query="HTTP status codes")`

5. **Implementing Versioning**
   - Situation: Need to evolve API without breaking clients
   - Query: `pos_search(content_type="standards", query="API versioning compatibility")`

6. **Handling Errors in APIs**
   - Situation: Designing error response format
   - Query: `pos_search(content_type="standards", query="API error handling")`

7. **Pagination/Filtering Design**
   - Situation: Need to implement pagination for collections
   - Query: `pos_search(content_type="standards", query="REST API pagination")`

### Query by Use Case

| Use Case | Example Query |
|----------|---------------|
| Starting API design | `pos_search(content_type="standards", query="how to design APIs")` |
| REST endpoint design | `pos_search(content_type="standards", query="REST API design")` |
| HTTP status codes | `pos_search(content_type="standards", query="HTTP status codes")` |
| API error messages | `pos_search(content_type="standards", query="API error handling")` |
| API versioning | `pos_search(content_type="standards", query="API versioning compatibility")` |
| Library API design | `pos_search(content_type="standards", query="library SDK API design")` |
...
- `standards/testing/integration-testing.md` - How to test APIs effectively
  → `pos_search(content_type="standards", query="integration testing")`
- `standards/testing/test-pyramid.md` - Testing strategy for API layers
  → `pos_search(content_type="standards", query="test pyramid API testing")`
...
- `standards/documentation/rag-content-authoring.md` - How to document for discoverability
  → `pos_search(content_type="standards", query="documentation standards")`

**Query workflow:**
1. **Before**: `pos_search(content_type="standards", query="API design principles")` → Learn universal principles
2. **During**: `pos_search(content_type="standards", query="REST API design")` → Apply to specific API type
3. **Testing**: `pos_search(content_type="standards", query="how to test APIs")` → Validate with tests
4. **After**: `pos_search(content_type="standards", query="production code checklist")` → Final quality check
