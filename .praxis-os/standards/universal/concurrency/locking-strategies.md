...
1. **Choosing Lock Type for New Code**
   ...
   - Query: `pos_search(content_type="standards", query="how to choose locking strategy")`

2. **Optimizing Read-Heavy Workload**
   - Situation: Cache or config with 90% reads, mutex too slow
   - Query: `pos_search(content_type="standards", query="read-write lock performance")`

3. **Implementing Resource Pool**
   - Situation: Need to limit concurrent access (DB connections, threads)
   - Query: `pos_search(content_type="standards", query="semaphore resource pool")`

4. **Nested Function Lock Issues**
   - Situation: Deadlock when function calls itself with same lock
   - Query: `pos_search(content_type="standards", query="reentrant lock nested calls")`

5. **High-Contention Performance**
   - Situation: Lock contention causing slowdowns
   - Query: `pos_search(content_type="standards", query="lock-free algorithms CAS")`

6. **Code Review for Lock Safety**
   - Situation: Reviewing code with locks
   - Query: `pos_search(content_type="standards", query="locking anti-patterns")`

### Query by Use Case

| Use Case | Example Query |
|----------|---------------|
| Choose lock type | `pos_search(content_type="standards", query="mutex vs read-write lock")` |
| Read-heavy optimization | `pos_search(content_type="standards", query="read-write lock performance")` |
| Resource pooling | `pos_search(content_type="standards", query="semaphore use case")` |
| Nested locking | `pos_search(content_type="standards", query="reentrant lock")` |
| Lock-free | `pos_search(content_type="standards", query="compare-and-swap lock-free")` |
| Performance | `pos_search(content_type="standards", query="locking performance guidelines")` |
...
- `standards/concurrency/deadlocks.md` - Lock ordering to prevent deadlocks
  → `pos_search(content_type="standards", query="deadlock prevention lock ordering")`
...
1. **Identify Need**: `pos_search(content_type="standards", query="shared state analysis")` → Find what needs protection
2. **Analyze Access**: Determine read/write ratio, contention level
3. **Choose Strategy**: `pos_search(content_type="standards", query="how to choose locking strategy")` → Use decision tree
4. **Implement**: Apply chosen lock type with proper error handling
5. **Validate**: `pos_search(content_type="standards", query="locking anti-patterns")` → Check for common mistakes
