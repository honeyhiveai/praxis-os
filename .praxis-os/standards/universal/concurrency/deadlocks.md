...
1. **System Hangs Indefinitely**
   ...
   - Query: `pos_search(content_type="standards", query="deadlock system hangs")`

2. **Implementing Multi-Lock Code**
   - Situation: Need to acquire multiple locks
   - Query: `pos_search(content_type="standards", query="how to prevent deadlocks")`

3. **Code Review for Lock Safety**
   - Situation: Reviewing code with multiple locks
   - Query: `pos_search(content_type="standards", query="deadlock prevention lock ordering")`

4. **Choosing Deadlock Prevention Strategy**
   - Situation: Deciding between lock ordering, timeout, lock-free
   - Query: `pos_search(content_type="standards", query="deadlock prevention strategies")`

5. **Debugging Concurrent System Freeze**
   - Situation: Production system hangs under load
   - Query: `pos_search(content_type="standards", query="how to detect deadlocks")`

6. **Understanding Coffman Conditions**
   - Situation: Learning deadlock theory
   - Query: `pos_search(content_type="standards", query="Coffman conditions deadlock")`

### Query by Use Case

| Use Case | Example Query |
|----------|---------------|
| Prevent deadlocks | `pos_search(content_type="standards", query="deadlock prevention")` |
| Lock ordering | `pos_search(content_type="standards", query="lock ordering prevent deadlock")` |
| Detect deadlocks | `pos_search(content_type="standards", query="how to detect deadlocks")` |
| Timeout strategy | `pos_search(content_type="standards", query="timeout deadlock prevention")` |
| Dining philosophers | `pos_search(content_type="standards", query="dining philosophers deadlock")` |
...
- `standards/testing/integration-testing.md` - Stress testing for deadlocks
  → `pos_search(content_type="standards", query="stress testing concurrency")`

**Query workflow for preventing deadlocks:**
1. **Learn Theory**: `pos_search(content_type="standards", query="Coffman conditions")` → Understand 4 conditions
2. **Identify Resources**: `pos_search(content_type="standards", query="shared state analysis")` → Find what needs locking
3. **Choose Strategy**: `pos_search(content_type="standards", query="deadlock prevention strategies")` → Select lock ordering (best)
4. **Implement**: Define global lock order, apply consistently
5. **Test**: `pos_search(content_type="standards", query="how to test for deadlocks")` → Stress test with many threads
