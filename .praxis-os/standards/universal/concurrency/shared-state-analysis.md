...
1. **Starting Concurrent Code Design**
   ...
   - Query: `pos_search(content_type="standards", query="how to identify shared state")`

2. **Code Review for Concurrency**
   - Situation: Reviewing code that uses threads/async
   - Query: `pos_search(content_type="standards", query="shared state analysis checklist")`

3. **Debugging Concurrency Bugs**
   - Situation: Intermittent failures, race conditions suspected
   - Query: `pos_search(content_type="standards", query="shared mutable state patterns")`

4. **Refactoring to Improve Thread Safety**
   - Situation: Have shared state, want to eliminate or protect it
   - Query: `pos_search(content_type="standards", query="how to refactor shared state")`

5. **Choosing Synchronization Approach**
   - Situation: Deciding between locks, immutability, message passing
   - Query: `pos_search(content_type="standards", query="shared state refactoring strategies")`

6. **Understanding Concurrency Failures**
   - Situation: "Why is my concurrent code breaking?"
   - Query: `pos_search(content_type="standards", query="shared mutable state root cause")`

### Query by Use Case

| Use Case | Example Query |
|----------|---------------|
| Identify shared state | `pos_search(content_type="standards", query="how to identify shared state")` |
| Classify state safety | `pos_search(content_type="standards", query="local vs shared state")` |
| Refactor away sharing | `pos_search(content_type="standards", query="eliminate shared state")` |
| Add synchronization | `pos_search(content_type="standards", query="synchronize shared state")` |
| Test concurrency | `pos_search(content_type="standards", query="test shared state issues")` |
| Code review checklist | `pos_search(content_type="standards", query="shared state code review")` |
...
- `standards/concurrency/race-conditions.md` - Race conditions from unsynchronized shared state
  → `pos_search(content_type="standards", query="race condition prevention")`
- `standards/concurrency/deadlocks.md` - Deadlocks from improper lock ordering
  → `pos_search(content_type="standards", query="deadlock prevention")`
- `standards/concurrency/locking-strategies.md` - Choosing locks to protect shared state
  → `pos_search(content_type="standards", query="locking strategies")`

**Testing Standards:**
- `standards/testing/integration-testing.md` - Testing concurrent code with shared state
  → `pos_search(content_type="standards", query="integration testing concurrency")`

**Query workflow for managing shared state:**
1. **Identify**: `pos_search(content_type="standards", query="how to identify shared state")` → Find all shared data
2. **Classify**: Determine if local, shared immutable, or shared mutable
3. **Analyze**: `pos_search(content_type="standards", query="escape analysis")` → Check if local data escapes
4. **Choose Strategy**: `pos_search(content_type="standards", query="how to refactor shared state")` → Eliminate, immutabilize, or synchronize
5. **Implement**: Apply chosen strategy (locks, atomics, immutable structures)
6. **Test**: `pos_search(content_type="standards", query="test shared state issues")` → Stress test with concurrency
7. **Review**: `pos_search(content_type="standards", query="shared state code review")` → Validate with checklist
