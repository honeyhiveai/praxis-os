...
1. **Debugging Intermittent Failures**
   ...
   - Query: `pos_search(content_type="standards", query="race condition intermittent failures")`

2. **Implementing Concurrent Code**
   - Situation: Writing multi-threaded or async code
   - Query: `pos_search(content_type="standards", query="how to prevent race conditions")`

3. **Code Review for Thread Safety**
   - Situation: Reviewing code that uses shared state
   - Query: `pos_search(content_type="standards", query="how to detect race conditions")`

4. **Production Bugs Under Load**
   - Situation: "Works on my machine, fails in production"
   - Query: `pos_search(content_type="standards", query="race condition symptoms")`

5. **Choosing Synchronization Strategy**
   - Situation: Deciding between locks, atomics, immutability
   - Query: `pos_search(content_type="standards", query="race condition prevention strategies")`

### Query by Use Case

| Use Case | Example Query |
|----------|---------------|
| Detect races | `pos_search(content_type="standards", query="how to detect race conditions")` |
| Prevent races | `pos_search(content_type="standards", query="race condition prevention")` |
| Test for races | `pos_search(content_type="standards", query="test for race conditions")` |
| Fix check-then-act | `pos_search(content_type="standards", query="check-then-act race condition")` |
| Thread safety | `pos_search(content_type="standards", query="thread safety patterns")` |
...
- `standards/concurrency/shared-state-analysis.md` - Identifying shared state
  → `pos_search(content_type="standards", query="shared state analysis")`
...
1. **Detect**: `pos_search(content_type="standards", query="how to detect race conditions")` → Identify shared state
2. **Analyze**: `pos_search(content_type="standards", query="shared state analysis")` → Determine access patterns
3. **Choose Strategy**: `pos_search(content_type="standards", query="race condition prevention")` → Select locks/atomics/immutability
4. **Implement**: `pos_search(content_type="standards", query="locking strategies")` → Apply synchronization
5. **Test**: `pos_search(content_type="standards", query="test for race conditions")` → Validate with stress tests
6. **Review**: `pos_search(content_type="standards", query="deadlock prevention")` → Ensure no new deadlocks
