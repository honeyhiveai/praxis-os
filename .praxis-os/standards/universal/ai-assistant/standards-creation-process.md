...
- [Scenario 1] → `pos_search(content_type="standards", query="[query phrase 1]")`
- [Scenario 2] → `pos_search(content_type="standards", query="[query phrase 2]")`
- [Scenario 3] → `pos_search(content_type="standards", query="[query phrase 3]")`
...
|-----------|---------------|
| **[Scenario 1]** | `pos_search(content_type="standards", query="[natural query 1]")` |
| **[Scenario 2]** | `pos_search(content_type="standards", query="[natural query 2]")` |
| **[Scenario 3]** | `pos_search(content_type="standards", query="[natural query 3]")` |
...
1. **Start with [this standard]** → `pos_search(content_type="standards", query="[topic]")` (this document)
2. **Learn [related topic 1]** → `pos_search(content_type="standards", query="[related topic 1]")` → `standards/[path]/[file].md`
3. **Understand [related topic 2]** → `pos_search(content_type="standards", query="[related topic 2]")` → `standards/[path]/[file].md`

**By Category:**

**[Category 1]:**
- `standards/[path]/[file].md` - [Description] → `pos_search(content_type="standards", query="[query]")`
...
**Query anytime:**
```python
pos_search(content_type="standards", query="[natural language query]")
```
...
```python
pos_search(content_type="standards", query="[topic] standards")
pos_search(content_type="standards", query="how to [do the thing]")
pos_search(content_type="standards", query="[related concept] guidelines")
```
...
```python
pos_search(content_type="standards", query="how to make content discoverable for agents")
pos_search(content_type="standards", query="RAG content authoring keywords query hooks")
pos_search(content_type="standards", query="avoid generic terms keyword pollution")
pos_search(content_type="standards", query="front-load critical information TL;DR")
```

**Query for query patterns (how agents search):**
```python
pos_search(content_type="standards", query="query construction content-specific phrases")
pos_search(content_type="standards", query="semantic search patterns effectiveness")
pos_search(content_type="standards", query="avoid generic questions use unique values")
```
...
pos_search(content_type="standards", query="[your content-specific phrase 1]")
pos_search(content_type="standards", query="[your content-specific phrase 2]")
pos_search(content_type="standards", query="[natural question from your query hooks]")
pos_search(content_type="standards", query="[domain-specific term unique to your topic]")
pos_search(content_type="standards", query="[problem your standard solves]")
```

**Should NOT return your standard:**
```python
# Test that generic terms don't cause pollution
pos_search(content_type="standards", query="workflow")  # Should return workflow standards, not yours
pos_search(content_type="standards", query="testing")   # Should return test standards, not yours
pos_search(content_type="standards", query="process")   # Should return process standards, not yours
```
...
pos_search(content_type="standards", query="how to [do what standard covers]")

# Angle 2: "What is" conceptual query  
pos_search(content_type="standards", query="what is [main concept]")

# Angle 3: Best practices query
pos_search(content_type="standards", query="[topic] best practices")

# Angle 4: Problem-solving query
pos_search(content_type="standards", query="when should I [use this]")

# Angle 5: Anti-pattern query
pos_search(content_type="standards", query="[topic] anti-patterns")
...
Verify it's indexed:
```python
pos_search(content_type="standards", query="[your standard topic]")
```
...
**Always query first**:
```python
pos_search(content_type="standards", query="[topic you're covering]")
```
...
- [ ] **Cross-references with queries** - `pos_search(content_type="standards", query="[topic]")` not just file links
...
pos_search(content_type="standards", query="how to [primary approach]")
pos_search(content_type="standards", query="what is [main concept]")
pos_search(content_type="standards", query="[action] best practices")
pos_search(content_type="standards", query="when should I [scenario]")
pos_search(content_type="standards", query="[topic] anti-patterns")
```
...
1. **Test current discoverability**:
   ```python
   pos_search(content_type="standards", query="[topic]")
   ```
...
- **Add deprecation notice** at top:
  ...
  Use pos_search(content_type="standards", query="[new topic]") for current guidance.
  ```
...
→ Query to find existing: `pos_search(content_type="standards", query="[topic]")`. Update existing or document why new one is needed.
...
| Situation | Example Query |
|-----------|---------------|
| **Creating standard** | `pos_search(content_type="standards", query="how to create standard")` |
| **Standard structure** | `pos_search(content_type="standards", query="standard structure template")` |
| **Quality criteria** | `pos_search(content_type="standards", query="what makes good standard")` |
| **When to create** | `pos_search(content_type="standards", query="when to write standard")` |
| **Standards vs specs** | `pos_search(content_type="standards", query="standard vs spec")` |
| **Maintenance** | `pos_search(content_type="standards", query="maintain standards")` |
| **Anti-patterns** | `pos_search(content_type="standards", query="standards creation mistakes")` |
| **Examples** | `pos_search(content_type="standards", query="good standard examples")` |
...
1. **Start with creation process** → `pos_search(content_type="standards", query="how to create standard")` (this document)
2. **Learn RAG optimization** → `pos_search(content_type="standards", query="RAG content authoring")` → `standards/documentation/rag-content-authoring.md`
3. **Understand specs** → `pos_search(content_type="standards", query="creating specs")` → `usage/creating-specs.md`
4. **Learn command language** → `pos_search(content_type="standards", query="command language")` → `standards/meta-workflow/command-language.md`

**By Category:**

**Documentation:**
- `standards/documentation/rag-content-authoring.md` - RAG optimization → `pos_search(content_type="standards", query="RAG content authoring")`
- `standards/documentation/readme-templates.md` - README patterns → `pos_search(content_type="standards", query="README templates")`
- `standards/documentation/code-comments.md` - Comment guidelines → `pos_search(content_type="standards", query="code comments")`

**Meta-Framework:**
- `standards/meta-workflow/command-language.md` - Command symbols → `pos_search(content_type="standards", query="command language")`
- `standards/meta-workflow/three-tier-architecture.md` - Content organization → `pos_search(content_type="standards", query="three tier architecture")`
- `standards/meta-workflow/validation-gates.md` - Quality gates → `pos_search(content_type="standards", query="validation gates")`

**Usage:**
- `usage/creating-specs.md` - Spec creation guide → `pos_search(content_type="standards", query="creating specs")`
- `usage/operating-model.md` - prAxIs OS principles → `pos_search(content_type="standards", query="operating model")`
