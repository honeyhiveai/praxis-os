...
- Before generating code → `pos_search(content_type="standards", query="pre-generation validation")`
- Import verification → `pos_search(content_type="standards", query="import verification")`
- Date handling → `pos_search(content_type="standards", query="date usage policy")`
...
|-----------|---------------|
| **Before generating code** | `pos_search(content_type="standards", query="pre-generation validation")` |
| **Multi-file tasks** | `pos_search(content_type="standards", query="validation checkpoints")` |
| **Import verification** | `pos_search(content_type="standards", query="import verification rules")` |
| **Date handling** | `pos_search(content_type="standards", query="date usage policy")` |
| **Clean state questions** | `pos_search(content_type="standards", query="when is clean state required")` |
| **Validation mistakes** | `pos_search(content_type="standards", query="common validation mistakes")` |
...
1. **Start with pre-generation validation** → `pos_search(content_type="standards", query="pre-generation validation")` (this document)
2. **Learn compliance protocol** → `pos_search(content_type="standards", query="compliance checking")` → `standards/ai-assistant/compliance-protocol.md`
...
4. **Master import verification** → `pos_search(content_type="standards", query="import verification")` → `standards/ai-safety/import-verification-rules.md`

**By Category:**

**AI Safety:**
- `standards/ai-safety/import-verification-rules.md` - Verify imports before use → `pos_search(content_type="standards", query="import verification rules")`
...
**AI Assistant:**
- `standards/ai-assistant/compliance-protocol.md` - Check standards before code → `pos_search(content_type="standards", query="compliance checking")`
- `standards/ai-assistant/commit-protocol.md` - Review and commit → `pos_search(content_type="standards", query="commit protocol")`
