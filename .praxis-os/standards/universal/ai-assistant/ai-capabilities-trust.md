...
# Query liberally:
pos_search(content_type="standards", query="how to handle race conditions")  # Query 1: Definition
pos_search(content_type="standards", query="where are race conditions handled in this project")  # Query 2: Location  
pos_search(content_type="standards", query="race condition testing best practices")  # Query 3: Testing
pos_search(content_type="standards", query="common race condition mistakes")  # Query 4: Error prevention
pos_search(content_type="standards", query="race condition debugging workflow")  # Query 5: Practical
...
```
Implementing authentication feature:
  1. pos_search(content_type="standards", query="authentication patterns")  # General
  2. pos_search(content_type="standards", query="JWT handling this project")  # Project-specific
  3. pos_search(content_type="standards", query="token refresh best practices")  # Best practice
  4. pos_search(content_type="standards", query="auth testing strategies")  # Testing
  5. pos_search(content_type="standards", query="common auth mistakes")  # Error prevention
  6. pos_search(content_type="standards", query="session management")  # Related area
