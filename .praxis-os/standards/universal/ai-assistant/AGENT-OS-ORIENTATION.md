...
pos_search(content_type="standards", query="grep-first reflex before file operations decision moment pause query")

# 2. Three-phase development  
pos_search(content_type="standards", query="user asks build something what do first discuss spec implement")

# 3. Code author role
pos_search(content_type="standards", query="code author role behavior implement query test lint complete")

# 4. Query construction
pos_search(content_type="standards", query="content-specific phrases unique values avoid generic structural")

# 5. Multi-angle querying (5-10 times)
pos_search(content_type="standards", query="single query syndrome 5-10 times multi-angle comprehensive discovery")

# 6. Workflow discovery
pos_search(content_type="standards", query="workflow discovery dynamic don't memorize workflow names query for task")

# 7. Indexed files explicit list
pos_search(content_type="standards", query=".praxis-os/standards indexed query not read universal workflows usage")

# 8. Practical examples (8 scenarios)
pos_search(content_type="standards", query="AI agent quickstart wrong right examples helper mode implementer concrete scenarios")

# 9. Workspace organization (where files go)
pos_search(content_type="standards", query="workspace organization ephemeral files design analysis scratch temporary git pollution")

# 10. AI capabilities and trust (fight inherited constraints)
pos_search(content_type="standards", query="AI capabilities versus human limitations trust systematic unlimited sessions")
```
...
- Starting new session → `pos_search(content_type="standards", query="prAxIs OS orientation")`
- User gives request → `pos_search(content_type="standards", query="prAxIs OS orientation")`  
- After 20+ messages → `pos_search(content_type="standards", query="prAxIs OS orientation")` (refresh pattern)
- Catching yourself in helper mode → `pos_search(content_type="standards", query="prAxIs OS orientation")`
...
```python
pos_search(content_type="standards", query="indexed query don't read .praxis-os/standards .praxis-os/workflows .praxis-os/usage specs")
```
...
pos_search(content_type="standards", query="how to implement authentication")
pos_search(content_type="standards", query="authentication security patterns")
pos_search(content_type="standards", query="JWT token management")
pos_search(content_type="standards", query="authentication testing strategies")
pos_search(content_type="standards", query="when to use workflows for auth")
...
- `.praxis-os/standards/` → Use `pos_search()` - **NEVER read_file()**
- `.praxis-os/workflows/` → Use `pos_search()` - **NEVER read_file()**
- `.praxis-os/usage/` → Use `pos_search()` - **NEVER read_file()**
...
| **Starting new session** | `pos_search(content_type="standards", query="prAxIs OS orientation")` |
| **User gives request** | `pos_search(content_type="standards", query="prAxIs OS orientation")` |
| **After 20+ messages** | `pos_search(content_type="standards", query="prAxIs OS orientation")` |
| **Catching yourself in helper mode** | `pos_search(content_type="standards", query="prAxIs OS orientation code author role")` |
| **Uncertain about querying** | `pos_search(content_type="standards", query="how many times should I query")` |
| **Forgot the pattern** | `pos_search(content_type="standards", query="prAxIs OS work pattern")` |
| **Orientation refresh** | `pos_search(content_type="standards", query="prAxIs OS orientation bootstrap queries")` |
...
1. **Start with orientation** → `pos_search(content_type="standards", query="prAxIs OS orientation")` (this document)
2. **Learn decision protocol** → `pos_search(content_type="standards", query="AI agent decision protocol")` → Loaded by Query 1
3. **Learn development process** → `pos_search(content_type="standards", query="user wants me to build something")` → Loaded by Query 2
4. **Learn query construction** → `pos_search(content_type="standards", query="query construction patterns")` → Loaded by Query 4
5. **See practical examples** → `pos_search(content_type="standards", query="AI agent quickstart")` → Loaded by Query 8

**By Category:**

**AI Assistant:**
- `standards/ai-assistant/agent-decision-protocol.md` → `pos_search(content_type="standards", query="inherited patterns efficiency paradox")`
- `standards/ai-assistant/agent-os-development-process.md` → `pos_search(content_type="standards", query="three phase development")`
- `standards/ai-assistant/query-construction-patterns.md` → `pos_search(content_type="standards", query="query construction")`

**Usage:**
- `usage/ai-agent-quickstart.md` → `pos_search(content_type="standards", query="AI agent quickstart examples")`
- `usage/operating-model.md` → `pos_search(content_type="standards", query="operating model")`
