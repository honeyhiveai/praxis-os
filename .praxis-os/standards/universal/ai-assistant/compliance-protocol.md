...
- Starting any task → `pos_search(content_type="standards", query="compliance checking")`
- Before code generation → `pos_search(content_type="standards", query="pre-task compliance")`
- Checking patterns → `pos_search(content_type="standards", query="how to find existing patterns")`
...
|-----------|---------------|
| **Starting any task** | `pos_search(content_type="standards", query="compliance checking protocol")` |
| **Before code generation** | `pos_search(content_type="standards", query="pre-task compliance checklist")` |
| **Finding patterns** | `pos_search(content_type="standards", query="how to find existing patterns")` |
| **Avoiding duplication** | `pos_search(content_type="standards", query="check for existing solutions")` |
| **Measuring compliance** | `pos_search(content_type="standards", query="compliance scoring")` |
| **Compliance failures** | `pos_search(content_type="standards", query="common compliance mistakes")` |
...
1. **Start with compliance protocol** → `pos_search(content_type="standards", query="compliance checking")` (this document)
2. **Learn pre-generation validation** → `pos_search(content_type="standards", query="pre-generation validation")` → `standards/ai-assistant/pre-generation-validation.md`
3. **Understand commit protocol** → `pos_search(content_type="standards", query="commit protocol")` → `standards/ai-assistant/commit-protocol.md`
4. **Master analysis methodology** → `pos_search(content_type="standards", query="analysis methodology")` → `standards/ai-assistant/analysis-methodology.md`

**By Category:**

**AI Assistant:**
- `standards/ai-assistant/pre-generation-validation.md` - What to validate before code → `pos_search(content_type="standards", query="pre-generation validation")`
- `standards/ai-assistant/commit-protocol.md` - Review and commit changes → `pos_search(content_type="standards", query="commit protocol")`
- `standards/ai-assistant/analysis-methodology.md` - Comprehensive analysis → `pos_search(content_type="standards", query="analysis methodology")`
...
**AI Safety:**
- `standards/ai-safety/production-code-checklist.md` - Code quality requirements → `pos_search(content_type="standards", query="production code checklist")`
- `standards/ai-safety/git-safety-rules.md` - Git safety → `pos_search(content_type="standards", query="git safety rules")`
- `standards/ai-safety/import-verification-rules.md` - Import verification → `pos_search(content_type="standards", query="import verification")`

**Meta-Framework:**
- `standards/meta-workflow/standards-creation-process.md` - Creating standards → `pos_search(content_type="standards", query="standards creation")`
