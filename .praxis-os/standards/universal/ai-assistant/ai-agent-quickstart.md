...
pos_search(content_type="standards", query="How should I implement JWT authentication in Python?")
pos_search(content_type="standards", query="What security considerations for API authentication?")
pos_search(content_type="standards", query="How do I structure auth middleware?")
pos_search(content_type="standards", query="Best practices for password hashing in Python?")
pos_search(content_type="standards", query="JWT token refresh patterns?")
pos_search(content_type="standards", query="Rate limiting for authentication endpoints?")
pos_search(content_type="standards", query="How should I test authentication flows?")
...
pos_search(content_type="standards", query="When should I use Redis vs in-memory caching?")
...
- ✅ Debug autonomously
- ✅ Query standards if needed: `pos_search(content_type="standards", query="How do I debug pytest failures?")`
...
```python
# AI queries standards
pos_search(content_type="standards", query="What makes robust error handling in authentication?")
pos_search(content_type="standards", query="How should I structure exception handling in Python?")
...
```python
# AI queries standards
pos_search(content_type="standards", query="How should I document API authentication?")
pos_search(content_type="standards", query="What documentation format for Python API?")
...
pos_search(content_type="standards", query="How do I profile Python API performance?")

# Query 2: What to look for
pos_search(content_type="standards", query="What are common API performance bottlenecks?")
...
pos_search(content_type="standards", query="How do I fix N+1 query problems in Python?")

# Query 4: Database optimization
pos_search(content_type="standards", query="Database indexing best practices?")

# Query 5: Caching patterns
pos_search(content_type="standards", query="How should I implement API response caching?")
...
# Query 6: Validation
pos_search(content_type="standards", query="How do I measure API performance improvements?")
...
```python
# AI queries standards
pos_search(content_type="standards", query="How should I structure authentication code?")
pos_search(content_type="standards", query="What are code organization best practices?")
pos_search(content_type="standards", query="When should I split files vs keep together?")
...
2. **Query Standards** (If Uncertain)
   - `pos_search(content_type="standards", query="How do I X?")`
...
1. **Starting a New Session**
   ...
   - Query: `pos_search(content_type="standards", query="AI agent quickstart")`

2. **Unsure How to Respond**
   - Situation: User makes request, unclear if I should implement or suggest
   - Query: `pos_search(content_type="standards", query="how should I respond to feature request")`

3. **Catching Wrong Behavior**
   - Situation: Noticed I'm suggesting instead of implementing
   - Query: `pos_search(content_type="standards", query="AI agent correct behavior patterns")`

4. **Handling Specific Scenarios**
   - Situation: Test failures, code review, documentation request, etc.
   - Query: `pos_search(content_type="standards", query="how to handle test failures autonomously")`

5. **Reinforcing Patterns Mid-Session**
   - Situation: Long session, want to maintain correct patterns
   - Query: `pos_search(content_type="standards", query="AI agent practical examples")`

6. **Teaching Others**
   - Situation: Onboarding new AI agents or documenting behavior
   - Query: `pos_search(content_type="standards", query="prAxIs OS correct AI behavior")`

### Query by Use Case

| Use Case | Example Query |
|----------|---------------|
| Session start | `pos_search(content_type="standards", query="AI agent quickstart")` |
| Feature request | `pos_search(content_type="standards", query="how to handle feature requests")` |
| Test failures | `pos_search(content_type="standards", query="handling test failures autonomously")` |
| Code review | `pos_search(content_type="standards", query="how to handle code review feedback")` |
| Documentation | `pos_search(content_type="standards", query="how to handle documentation requests")` |
| Performance issues | `pos_search(content_type="standards", query="handling performance issues")` |
| Refactoring | `pos_search(content_type="standards", query="how to handle refactoring")` |
| Behavior check | `pos_search(content_type="standards", query="correct AI agent behavior")` |
...
- `usage/operating-model.md` - Detailed role definition and boundaries
  → `pos_search(content_type="standards", query="prAxIs OS operating model")`
...
- `standards/ai-safety/production-code-checklist.md` - Quality requirements for all code
  → `pos_search(content_type="standards", query="production code quality checklist")`
...
**Query workflow:**
1. **Session Start**: `pos_search(content_type="standards", query="prAxIs OS orientation")` → Understand philosophy
...
pos_search(content_type="standards", query="practical AI agent examples")
...
pos_search(content_type="standards", query="how to X")  # Your actual question
```
