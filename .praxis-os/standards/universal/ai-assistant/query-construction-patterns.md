...
**This standard defines how AI assistants should construct effective `pos_search()` queries to discover prAxIs OS content reliably.**
...
**Winning Pattern:**
```python
✅ pos_search(content_type="standards", query="specific phrase from target section unique values")
❌ pos_search(content_type="standards", query="what is concept name")
```
...
pos_search(content_type="standards", query="widget sprocket gadget doohickey thingamajig")
```

**❌ Bad Pattern:**
```python
# Generic table structure terms
pos_search(content_type="standards", query="table rows columns")
```
...
pos_search(content_type="standards", query="initialize connection validate schema transform payload")
```

**❌ Bad Pattern:**
```python
# Generic process question
pos_search(content_type="standards", query="how to process data")
```
...
pos_search(content_type="standards", query="reviewing output approving changes fixing edge cases")
```

**❌ Bad Pattern:**
```python
# Structural keywords
pos_search(content_type="standards", query="INCLUDES EXCLUDES list items")
```
...
pos_search(content_type="standards", query="P Q R variables multiply divide result")
```

**❌ Bad Pattern:**
```python
# Generic math question
pos_search(content_type="standards", query="calculation formula")
```
...
pos_search(content_type="standards", query="start conservative 2.5x factor 12-15% threshold")
```

**❌ Bad Pattern:**
```python
# Generic concept
pos_search(content_type="standards", query="calibration guidelines recommendations")
```
...
pos_search(content_type="standards", query="template format example Baseline Enhanced Comparison")
```

**❌ Bad Pattern:**
```python
# Too generic
pos_search(content_type="standards", query="example template")
```
...
**❌ Fails:**
```python
pos_search(content_type="standards", query="what is dependency injection")
pos_search(content_type="standards", query="how to handle errors")
pos_search(content_type="standards", query="best practices for testing")
```
...
**✅ Fix:**
```python
pos_search(content_type="standards", query="constructor injection setter injection field injection")
pos_search(content_type="standards", query="exception wrapping context preservation stack trace")
pos_search(content_type="standards", query="arrange act assert given when then")
```
...
**❌ Fails:**
```python
pos_search(content_type="standards", query="INCLUDES EXCLUDES")
pos_search(content_type="standards", query="Step 1 Step 2 Step 3")
pos_search(content_type="standards", query="Section Header Subsection")
```
...
**✅ Fix:**
```python
pos_search(content_type="standards", query="actual activity phrases from content")
pos_search(content_type="standards", query="initialize validate transform send")
pos_search(content_type="standards", query="specific topic domain terminology")
```
...
**❌ Fails:**
```python
pos_search(content_type="standards", query="testing")
pos_search(content_type="standards", query="database")
pos_search(content_type="standards", query="performance")
```
...
**✅ Fix:**
```python
pos_search(content_type="standards", query="mock patch stub fake test double")
pos_search(content_type="standards", query="transaction isolation rollback deadlock")
pos_search(content_type="standards", query="memory cpu latency throughput bottleneck")
```
...
**❌ Fails:**
```python
pos_search(content_type="standards", query="explain concept name")
pos_search(content_type="standards", query="definition of term")
pos_search(content_type="standards", query="what does X mean")
```
...
**✅ Fix:**
```python
pos_search(content_type="standards", query="definition terminology example usage pattern")
pos_search(content_type="standards", query="key phrase from definition unique to concept")
pos_search(content_type="standards", query="symptoms causes solutions prevention")
```
...
pos_search(content_type="standards", query="widget sprocket gadget complexity comparison")
```
...
pos_search(content_type="standards", query="latency 50ms 95th percentile 200ms 99th")

# Target: Version-specific guidance
pos_search(content_type="standards", query="Python 3.11 3.12 match case structural pattern")
```
...
pos_search(content_type="standards", query="mutex lock atomic compare-and-swap memory order")

# Target: Dependency injection patterns
pos_search(content_type="standards", query="constructor injection container autowire lifecycle")

# Target: Testing strategies
pos_search(content_type="standards", query="unit integration end-to-end pyramid trophy")
```
...
pos_search(content_type="standards", query="parse validate check conflicts verify completeness")

# Target: Error handling flow
pos_search(content_type="standards", query="catch wrap log rethrow context preserve")

# Target: Review checklist
pos_search(content_type="standards", query="verify test document approve merge deploy")
```
...
```python
results = pos_search(query, n_results=3)
...
```markdown
**Working Query:**
pos_search(content_type="standards", query="widget sprocket gadget complexity multiplier")
...
```python
pos_search(content_type="standards", query="INCLUDES EXCLUDES list")
```
...
```python
pos_search(content_type="standards", query="what counts as active time")
```
...
Before running `pos_search()`, verify:
...
|-----------|---------------|
| **Constructing new query** | `pos_search(content_type="standards", query="query construction patterns best practices")` |
| **Query not finding content** | `pos_search(content_type="standards", query="why query fails content-specific phrases")` |
| **Choosing query strategy** | `pos_search(content_type="standards", query="table procedure list formula query patterns")` |
| **Debugging low relevance** | `pos_search(content_type="standards", query="query anti-patterns structural keywords")` |
| **Learning from examples** | `pos_search(content_type="standards", query="case study query optimization")` |
| **Testing query effectiveness** | `pos_search(content_type="standards", query="test query relevance score refine")` |
...
**Query anytime:**
```python
pos_search(content_type="standards", query="query construction patterns")
pos_search(content_type="standards", query="content-specific phrases semantic search")
pos_search(content_type="standards", query="query anti-patterns fails")
```
