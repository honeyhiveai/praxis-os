...
|-----------|---------------|
| **New integration** | `pos_search(content_type="standards", query="import verification")` |
| **Third-party package** | `pos_search(content_type="standards", query="how to verify imports")` |
| **Module not found error** | `pos_search(content_type="standards", query="how to find correct import paths")` |
| **SDK integration** | `pos_search(content_type="standards", query="verify package imports")` |
| **Import errors** | `pos_search(content_type="standards", query="import errors")` |
| **Package structure** | `pos_search(content_type="standards", query="discover package structure")` |
| **Hallucinated imports** | `pos_search(content_type="standards", query="AI import errors")` |
| **Before coding** | `pos_search(content_type="standards", query="2 minute rule imports")` |
...
1. **Start with verification** → `pos_search(content_type="standards", query="import verification")` (this document)
2. **Learn production checklist** → `pos_search(content_type="standards", query="production code checklist")` → `standards/ai-safety/production-code-checklist.md`
3. **Understand testing** → `pos_search(content_type="standards", query="integration testing")` → `standards/testing/integration-testing.md`
4. **Learn code documentation** → `pos_search(content_type="standards", query="code comments")` → `standards/documentation/code-comments.md`
...
- `standards/ai-safety/credential-file-protection.md` - File protection rules → `pos_search(content_type="standards", query="credential file protection")`
- `standards/ai-safety/date-usage-policy.md` - Date handling → `pos_search(content_type="standards", query="date usage policy")`
- `standards/ai-safety/git-safety-rules.md` - Git operations → `pos_search(content_type="standards", query="git safety rules")`

**Testing:**
- `standards/testing/integration-testing.md` - Integration test patterns → `pos_search(content_type="standards", query="integration testing")`
- `standards/testing/test-doubles.md` - Test isolation → `pos_search(content_type="standards", query="test doubles")`

**Documentation:**
- `standards/documentation/code-comments.md` - Code documentation → `pos_search(content_type="standards", query="code comments")`
- `standards/documentation/api-documentation.md` - API docs → `pos_search(content_type="standards", query="API documentation")`
