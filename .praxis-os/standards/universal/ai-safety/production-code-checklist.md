...
- Before starting new feature → `pos_search(content_type="standards", query="production code checklist")`
- During code review → `pos_search(content_type="standards", query="code quality requirements")`
- When adding dependencies → `pos_search(content_type="standards", query="dependency version management")`
- When handling failures → `pos_search(content_type="standards", query="graceful degradation patterns")`
- When managing state → `pos_search(content_type="standards", query="concurrency thread safety")`
...
pos_search(content_type="standards", query="thread safety concurrency patterns")

# For graceful degradation  
pos_search(content_type="standards", query="failure modes graceful degradation")

# For system architecture
pos_search(content_type="standards", query="dependency injection architecture patterns")

# For testing strategy
pos_search(content_type="standards", query="test pyramid unit integration")
```
...
# During code review
pos_search(content_type="standards", query="code quality standards AI")

# When debugging quality issues
pos_search(content_type="standards", query="common code quality mistakes")

# When onboarding to prAxIs OS
pos_search(content_type="standards", query="prAxIs OS code standards")
```
