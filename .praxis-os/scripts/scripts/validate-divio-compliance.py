...
class DivioValidator:
    ...
    def validate_all(self) -> List[FileResult]:
        ...
        if not md_files:
            print(f"{Colors.YELLOW}Warning: No markdown files found in {self.content_dir}{Colors.RESET}")
            return []
    ...
    def validate_file(self, file_path: Path) -> FileResult:
        ...
        compliance_score = max(0, (total_checks - violations_count) / total_checks * 100) if total_checks > 0 else 0
    ...
    def _validate_content_patterns(self, content: str, doc_type: str) -> List[Violation]:
        """Validate content patterns based on doc type"""
    ...
    def _validate_tutorial(self, content: str) -> List[Violation]:
        ...
        if not re.search(r'(?i)(learning|learn|objectives?|goals?|you will learn)', content):
            violations.append(Violation(
        ...
        if not re.search(r'(?i)(what (you|you\'ve) learned|summary|conclusion|recap)', content):
            violations.append(Violation(
    ...
    def _validate_how_to(self, content: str) -> List[Violation]:
        ...
        if not re.search(r'(?i)(this (guide|how-to) (shows?|explains?|demonstrates?)|problem|solution|goal)', content[:500]):
            violations.append(Violation(
        ...
        if not re.search(r'(?i)(prerequisite|requirement|before you begin|you (will )?need)', content):
            violations.append(Violation(
    ...
    def _validate_reference(self, content: str) -> List[Violation]:
        ...
        has_structure = bool(
            re.search(r'\|.*\|', content) or  # Tables
            re.search(r'^[-*+]\s+', content, re.MULTILINE) or  # Lists
    ...
    def _validate_explanation(self, content: str) -> List[Violation]:
        ...
        if not re.search(r'(?i)(background|context|why|history|motivation)', content[:1000]):
            violations.append(Violation(
        ...
        if not re.search(r'(?i)(trade-?off|advantage|disadvantage|benefit|drawback|comparison|versus|vs\.)', content):
            violations.append(Violation(
    ...
    def print_results(self, strict: bool = False):
        ...
        for result in self.results:
            score_color = Colors.GREEN if result.compliance_score >= 90 else Colors.YELLOW if result.compliance_score >= 70 else Colors.RED
            print(f"\n{Colors.BOLD}{result.file_path}{Colors.RESET}")
            print(f"  Doc Type: {result.doc_type or 'MISSING'}")
            print(f"  Compliance: {score_color}{result.compliance_score:.1f}%{Colors.RESET}")
            
            if result.violations:
                print(f"  Violations:")
                for v in result.violations:
                    severity_color = Colors.RED if v.severity == 'error' else Colors.YELLOW
                    print(f"    {severity_color}[{v.severity.upper()}]{Colors.RESET} {v.message}")
                    if v.remediation:
        ...
        summary_color = Colors.GREEN if avg_compliance >= 90 else Colors.YELLOW if avg_compliance >= 70 else Colors.RED
        ...
        print(f"Status: {Colors.GREEN if threshold_met else Colors.RED}{'PASS' if threshold_met else 'FAIL'}{Colors.RESET}")
    ...
    def generate_report(self, output_path: str = 'divio-compliance-report.md'):
        ...
        with open(output_path, 'w') as f:
            ...
            total_files = len(self.results)
            passed_files = len([r for r in self.results if r.passed])
            avg_compliance = sum(r.compliance_score for r in self.results) / total_files if total_files > 0 else 0
            ...
            for result in sorted(self.results, key=lambda r: r.compliance_score, reverse=True):
                status = '✅' if result.passed else '❌'
                f.write(f'{status} **{result.file_path}** - {result.compliance_score:.1f}%\n')
...
def main():
    ...
    parser.add_argument('--strict', action='store_true',
                       help='Require 100%% compliance (default: 90%%)')
    parser.add_argument('--report', action='store_true',
                       help='Generate markdown compliance report')
    ...
    if validator.results:
        ...
        avg_compliance = sum(r.compliance_score for r in validator.results) / total_files
