...
class TestGraphIndexInitialization:
    ...
    def test_initialization_basic(self, tmp_path):
        ...
        assert (
            index.db_path
            == tmp_path / ".cache" / "indexes" / "code" / "graph.duckdb"
        )
        # ast_extractor is initialized directly (no lazy loading)
        assert index.ast_extractor is not None
...
class TestGraphIndexSchema:
    ...
    def test_schema_has_relationships_table(self, tmp_path):
        ...
        assert hasattr(index, "db_connection")  # Changed from _ensure_connection
...
class TestGraphIndexUpdate:
    ...
    def test_update_has_correct_interface(self, tmp_path):
        ...
        assert "file_paths" in sig.parameters  # Changed from changed_files
...
class TestGraphIndexHealthAndStats:
    ...
    def test_health_check_healthy(self, tmp_path):
        """Test health check when index is healthy (has data)."""
        config = create_graph_config()
        index = GraphIndex(config, tmp_path)

        # Insert some test data to make index healthy (need both symbols AND relationships)
        conn = index.db_connection.get_connection()
        conn.execute("INSERT INTO symbols (id, name, type, file_path, line_number, language) VALUES (1, 'caller_func', 'function', '/test.py', 5, 'python')")
        conn.execute("INSERT INTO symbols (id, name, type, file_path, line_number, language) VALUES (2, 'called_func', 'function', '/test.py', 10, 'python')")
        conn.execute("INSERT INTO relationships (id, from_symbol_id, to_symbol_id, relationship_type) VALUES (1, 1, 2, 'calls')")
        conn.execute("INSERT INTO ast_nodes (id, file_path, language, node_type, symbol_name, start_line, end_line, parent_id) VALUES (1, '/test.py', 'python', 'function_definition', 'caller_func', 5, 8, NULL)")
...
class TestGraphIndexIntegration:
    ...
    def test_graph_index_separate_from_code_index(self, tmp_path):
        ...
        assert "graph.duckdb" in str(index.db_path)  # Changed from call_graph.duckdb
