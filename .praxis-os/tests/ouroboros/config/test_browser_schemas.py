...
class TestBrowserConfigDefaults:
    ...
    def test_browser_config_defaults(self):
        ...
        # Note: screenshot_dir was removed from BrowserConfig schema
...
class TestBrowserConfigCustomValues:
    ...
    def test_browser_config_custom_screenshot_dir(self):
        """BrowserConfig no longer has screenshot_dir field - test removed."""
        # Note: screenshot_dir was removed from BrowserConfig schema
        # This test is kept for historical reference but now just tests basic config
        config = BrowserConfig()
        assert config.browser_type == "chromium"
...
class TestBrowserConfigSerialization:
    ...
    def test_browser_config_to_dict(self):
        ...
        config = BrowserConfig(
            ...
            # Note: screenshot_dir removed from schema
        )
...
class TestBrowserTypeValidation:
    ...
    def test_browser_type_strips_whitespace(self):
        """BrowserConfig validates against regex pattern - whitespace not allowed."""
        # Pattern validation now prevents whitespace instead of stripping it
        with pytest.raises(ValidationError, match="String should match pattern"):
            BrowserConfig(browser_type=" chromium ")
