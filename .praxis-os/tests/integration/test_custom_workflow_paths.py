...
class TestCustomWorkflowPaths(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.base_path = Path(self.temp_dir) / ".praxis-os"
    ...
    def test_load_paths_no_config(self):
        ...
        # Should use defaults (new defaults use .praxis-os/ prefix)
        project_root = self.base_path.parent
        assert paths["standards_path"] == project_root / ".praxis-os" / "standards"
        assert paths["usage_path"] == project_root / ".praxis-os" / "usage"
        assert paths["workflows_path"] == project_root / ".praxis-os" / "workflows"
    ...
    def test_load_paths_from_config(self):
        ...
        project_root = self.base_path.parent
        assert (
            paths["standards_path"] == project_root / ".praxis-os-source" / "standards"
        )
        assert paths["usage_path"] == project_root / ".praxis-os-source" / "usage"
        assert (
            paths["workflows_path"] == project_root / ".praxis-os-source" / "workflows"
        )
    ...
    def test_load_paths_partial_config(self):
        ...
        # Others use defaults (new defaults use .praxis-os/ prefix)
        assert paths["standards_path"] == project_root / ".praxis-os" / "standards"
        assert paths["usage_path"] == project_root / ".praxis-os" / "usage"
