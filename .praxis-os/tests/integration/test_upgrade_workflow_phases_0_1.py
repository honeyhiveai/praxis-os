...
def mock_target_installation(tmp_path):
    """Create a complete mock .praxis-os installation."""
    target = tmp_path / "project" / ".praxis-os"
    ...
    config = {
        "version": "0.9.0",
        "features": {"browser_tools": True, "rag_search": True},
    }
...
class TestPhase0PreFlightChecks:
    ...
    def test_complete_phase_0_validation(
        self, mock_source_repo, mock_target_installation
    ):
        ...
        target_result = validator.validate_target_structure(
            str(mock_target_installation)
        )
    ...
    def test_phase_0_fails_on_dirty_git(
        self, mock_source_repo, mock_target_installation
    ):
...
class TestPhases0And1Integration:
    ...
    def test_complete_phases_0_and_1_flow(
        self, mock_source_repo, mock_target_installation
    ):
        ...
        concurrent_result = validator.check_for_concurrent_upgrades(str(lock_file))
        assert (
            concurrent_result["no_concurrent_workflows"] is True
        ), "Phase 0 concurrent check failed"
    ...
    def test_rollback_capability_after_phase_1(self, mock_target_installation):
        ...
        (mock_target_installation / "mcp_server" / "new_file.py").write_text(
            "# New file"
        )
