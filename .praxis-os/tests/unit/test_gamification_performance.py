...
from mcp_server.core.session_id_extractor import extract_session_id_from_context, hash_session_id
...
def test_prepend_generator_latency_under_10ms():
    ...
    assert p95_latency <= 10.0, f"Prepend generator p95 latency {p95_latency:.2f}ms exceeds 10ms"
...
def test_end_to_end_latency_under_20ms():
    ...
    assert p95_latency <= 20.0, f"End-to-end p95 latency {p95_latency:.2f}ms exceeds 20ms"
...
def test_memory_usage_100_sessions_under_100kb():
    ...
    assert memory_kb <= 100.0, f"Memory usage {memory_kb:.2f}KB exceeds 100KB for 100 sessions"
...
def test_memory_usage_1000_sessions_under_1mb():
    ...
    assert memory_mb <= 1.0, f"Memory usage {memory_mb:.2f}MB exceeds 1MB for 1,000 sessions"
...
def test_prepend_token_count_average_efficient():
    ...
    for i in range(100):
        ...
        for j in range(num_queries):
            tracker.record_query(session_id, f"Sample query {j} for testing token budget")
    ...
    assert avg_tokens <= 60, f"Average tokens {avg_tokens:.1f} exceeds efficient target of 60"
...
def test_sustained_load_100_sessions_10_queries():
    ...
    assert p95_latency <= 20.0, f"Sustained load p95 latency {p95_latency:.2f}ms exceeds 20ms"
...
def test_burst_load_1000_queries_single_session():
    ...
    assert last_100_p95 <= first_100_p95 * 2, \
        f"Burst load degraded: first p95={first_100_p95:.2f}ms, last p95={last_100_p95:.2f}ms"
    print(f"Burst load: first 100 p95={first_100_p95:.2f}ms, last 100 p95={last_100_p95:.2f}ms")
