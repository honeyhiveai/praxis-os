...
def test_hash_session_id_produces_16_char_hex():
    ...
    for session_id in test_session_ids:
        ...
        assert re.match(
            r"^[0-9a-f]{16}$", hashed
        ), f"Hashed ID '{hashed}' is not valid hex"
...
def test_hash_session_id_collision_resistance():
    ...
    assert len(set(hashes)) == len(
        hashes
    ), f"Hash collisions detected: {len(set(hashes))} unique hashes for {len(hashes)} inputs"
...
def test_log_privacy_no_plain_session_ids(caplog):
    ...
    for record in caplog.records:
        ...
        assert (
            plain_session_id not in message
...
def test_error_handling_no_implementation_details(caplog):
    ...
    with caplog.at_level(logging.ERROR):
        ...
        with patch.object(
            tracker, "_session_stats", side_effect=RuntimeError("Internal error")
        ):
            try:
    ...
    for record in caplog.records:
        if record.levelno == logging.ERROR:
            ...
            assert (
                "/Users/" not in message or "mcp_server" in message
            )  # Stack traces are OK
