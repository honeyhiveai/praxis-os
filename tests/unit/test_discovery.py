...
class TestDiscoverMCPServer:
    ...
    def agent_os_path(self, tmp_path):
        """Create a temporary .agent-os directory."""
    ...
    def test_discover_mcp_server_auto_find_agent_os(self, tmp_path, monkeypatch):
        """Test discovery automatically finds .agent-os in current directory."""
    ...
    def test_discover_mcp_server_auto_find_agent_os_not_found(
        ...
        """Test discovery returns None when .agent-os not found."""
        empty_dir = tmp_path / "empty"
        empty_dir.mkdir()
        monkeypatch.chdir(empty_dir)

        url = discover_mcp_server()  # No .agent-os in hierarchy
        assert url is None
...
class TestFindAgentOsDirectory:
    ...
    def test_find_in_current_directory(self, tmp_path, monkeypatch):
        """Test finding .agent-os in current directory."""
        agent_os = tmp_path / ".agent-os"
        agent_os.mkdir()
    ...
    def test_find_in_parent_directory(self, tmp_path, monkeypatch):
        """Test finding .agent-os in parent directory."""
        agent_os = tmp_path / ".agent-os"
        agent_os.mkdir()
    ...
    def test_not_found(self, tmp_path, monkeypatch):
        """Test returns None when .agent-os not found."""
        empty_dir = tmp_path / "empty"
        empty_dir.mkdir()
        monkeypatch.chdir(empty_dir)
    ...
    def test_stops_at_home_directory(self, tmp_path, monkeypatch):
        ...
        # Mock Path.home() to return a directory that doesn't contain .agent-os
        mock_home = tmp_path / "fake_home"
        mock_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: mock_home)

        result = _find_agent_os_directory()
        # Should not find .agent-os (it doesn't exist in the test hierarchy)
        assert result is None
