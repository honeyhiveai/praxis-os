...
class TestValidateSourceRepo:
    ...
    def test_validate_source_repo_not_agent_os(self, validator, tmp_path):
        """Test validation fails for non-praxis-os repository."""
        # Create directory without mcp_server
        not_agent_os = tmp_path / "not-praxis-os"
        not_agent_os.mkdir()
        ...
        assert "not an praxis-os-enhanced repository" in result["errors"][0].lower()
