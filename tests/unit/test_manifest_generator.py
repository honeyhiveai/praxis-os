...
class TestCalculateChecksum:
    ...
    def test_binary_file_checksum(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            expected = hashlib.sha256(b'\x00\x01\x02\x03\x04\x05\xff\xfe\xfd').hexdigest()
            actual = calculate_checksum(test_file)
    ...
    def test_large_file_memory_efficiency(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            assert all(c in '0123456789abcdef' for c in checksum), "Checksum should be hexadecimal"
    ...
    def test_checksum_length(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            assert len(checksum) == 64, f"SHA-256 checksum should be 64 chars, got {len(checksum)}"
...
class TestGetLastModifiedDate:
    ...
    def test_untracked_file_falls_back_to_filesystem(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            parsed_date = datetime.strptime(date_str, "%Y-%m-%d")
            now = datetime.now()
            assert abs((now - parsed_date).days) <= 1, \
    ...
    def test_returns_iso_date_format(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            assert len(date_str) == 10, f"Expected 10 chars, got {len(date_str)}: {date_str}"
