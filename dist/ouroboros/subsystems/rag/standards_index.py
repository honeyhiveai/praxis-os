"""Standards Index: Hybrid search (Vector + FTS + RRF) for standards content.

This index provides multi-strategy search over prAxIs OS standards:
...
class StandardsIndex(BaseIndex):
    """Hybrid search index for standards content.
    ...
    def __init__(self, config: StandardsIndexConfig, base_path: Path):
        ...
        logger.info("StandardsIndex initialized (lazy-load mode)")
        ...
        if self._table is None:
            ...
            try:
                ...
                raise IndexError(
                    ...
                    how_to_fix="Build index first using IndexManager.rebuild_index('standards')"
                ) from e
    ...
    def _ensure_embedding_model(self):
        ...
        if self._embedding_model is None:
            try:
                ...
                raise ActionableError(
                    what_failed=f"Load embedding model ({self.config.vector.model})",
                    why_failed=str(e),
                    how_to_fix="Check model name in config. Valid models: BAAI/bge-small-en-v1.5, BAAI/bge-base-en-v1.5, sentence-transformers/all-MiniLM-L6-v2"
                ) from e
    ...
    def build(self, source_paths: List[Path], force: bool = False) -> None:
        ...
        if "standards" in existing_tables and force:
            ...
            self._db.drop_table("standards")
        
        logger.info("Creating standards table with %d chunks", len(chunks))
        self._table = self._db.create_table("standards", data=chunks)
    ...
    def health_check(self) -> HealthStatus:
        ...
        3. Scalar indexes exist (if enabled)
        ...
        try:
            ...
            if self.config.metadata_filtering and self.config.metadata_filtering.enabled:
                try:
                    assert self._table is not None  # Type guard: already checked above
                    indexes = self._table.list_indices()
