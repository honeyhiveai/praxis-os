"""Code Index: Semantic search using CodeBERT embeddings in LanceDB.

This index provides semantic code search using CodeBERT/GraphCodeBERT embeddings.
...
Graph traversal (call graphs, dependencies) is handled by GraphIndex (Task 3.4).
...
class CodeIndex(BaseIndex):
    """Semantic code search index using LanceDB.
    ...
    def __init__(self, config: CodeIndexConfig, base_path: Path):
        ...
        # Lazy-load dependencies (expensive imports)
        self._db: Any = None
        self._table: Any = None
        self._embedding_model: Optional["SentenceTransformer"] = None
        self._reranker: Any = None
        
        logger.info("CodeIndex initialized (lazy-load mode)")
    
    def _ensure_db(self):
        """Ensure LanceDB connection is established (lazy initialization)."""
        if self._db is None:
            try:
                import lancedb
                self._db = lancedb.connect(str(self.index_path))
                logger.info("Connected to LanceDB at %s", self.index_path)
                ...
                raise ActionableError(
                    ...
                    why_failed=str(e),
                    how_to_fix="Check that cache/indexes/ directory is writable"
                ) from e
    ...
    def _ensure_table(self):
        ...
        if self._table is None:
            self._ensure_db()
            assert self._db is not None  # Type guard: _ensure_db() always sets this
            try:
                self._table = self._db.open_table("code")
                logger.info("Opened code table")
            except Exception as e:
                raise IndexError(
                    what_failed="Open code table",
                    why_failed="Table does not exist. Index not built yet.",
                    how_to_fix="Build index first using IndexManager.rebuild_index('code')"
                ) from e
    
    def _ensure_embedding_model(self):
        """Ensure code embedding model is loaded (lazy initialization)."""
        if self._embedding_model is None:
            try:
                from sentence_transformers import SentenceTransformer
                
                # Use code-optimized model (microsoft/codebert-base or similar)
                # For now, use a general model that works reasonably on code
                # TODO: Switch to microsoft/codebert-base or microsoft/graphcodebert-base
                model_name = self.config.vector.model
                logger.info("Loading code embedding model: %s", model_name)
                embedding_model = SentenceTransformer(model_name)
                self._embedding_model = embedding_model  # Assign after creation
                ...
                raise ActionableError(
                    what_failed=f"Load code embedding model ({self.config.vector.model})",
                    why_failed=str(e),
                    how_to_fix="Check model name in config. For code: microsoft/codebert-base, microsoft/graphcodebert-base"
                ) from e
    ...
    def build(self, source_paths: List[Path], force: bool = False) -> None:
        ...
        assert self._embedding_model is not None  # Type guard: _ensure_embedding_model() always sets this
        ...
        if "code" in existing_tables and force:
            ...
            self._db.drop_table("code")
        
        logger.info("Creating code table with %d chunks", len(chunks))
        self._table = self._db.create_table("code", data=chunks)
    ...
    def _chunk_file(self, file_path: Path) -> List[Dict[str, Any]]:
        ...
        - TODO: AST-aware chunking at function/class boundaries (Task 3.5)
    ...
    def search(
        ...
        """
        self._ensure_table()
        assert self._table is not None  # Type guard: _ensure_table() always sets this
        self._ensure_embedding_model()
        assert self._embedding_model is not None  # Type guard: _ensure_embedding_model() always sets this
    ...
    def _vector_search(
        ...
        results_raw = search_query.limit(limit).to_list()
        results: List[Dict[str, Any]] = [dict(r) for r in results_raw] if results_raw else []
    ...
    # Graph methods (stubs for now - implemented in GraphIndex, Task 3.4)
    def find_callers(self, symbol_name: str, max_depth: int = 10) -> List[Dict[str, Any]]:
        """Find who calls this symbol.
        
        NOTE: This is handled by GraphIndex (Task 3.4).
        CodeIndex provides semantic search only.
        """
        logger.warning("find_callers() should be called on GraphIndex, not CodeIndex")
        return []
    ...
    def find_dependencies(self, symbol_name: str, max_depth: int = 10) -> List[Dict[str, Any]]:
        ...
        CodeIndex provides semantic search only.
        """
        logger.warning("find_dependencies() should be called on GraphIndex, not CodeIndex")
        return []
    ...
    def find_call_paths(
        ...
        CodeIndex provides semantic search only.
        """
        logger.warning("find_call_paths() should be called on GraphIndex, not CodeIndex")
        return []
