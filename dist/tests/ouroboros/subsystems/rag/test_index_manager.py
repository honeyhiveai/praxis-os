...
class TestIndexManager:
    ...
    def test_route_action_search_standards(self, minimal_indexes_config, tmp_path):
        ...
        # Mock the standards index search method and build_status
        ...
        manager._indexes["standards"].build_status = MagicMock(
            return_value=BuildStatus(
    ...
    def test_route_action_search_code_not_built(self, minimal_indexes_config, tmp_path):
        ...
        # With build status integration, route_action now returns an error dict
        # instead of raising an exception when index is not built
        result = manager.route_action(action="search_code", query="test query")

        assert result["status"] == "error"
        assert result["error"] == "Index not built"
        assert "code" in result["message"]
        assert "build_status" in result
        assert result["build_status"]["state"] != "BUILT"
