...
def format_config_summary(config: dict, languages: List[str]) -> str:
    ...
    if "code" in config["indexes"]:
        ...
        lines.append(f"  ✓ Code search ({len(code['languages'])} languages: {lang_str})")
    ...
    lines.append(f"  ✓ Standards ({patterns}) - {standards['debounce_seconds']}s debounce")
    ...
    if "code" in config["monitoring"]["file_watcher"]["watched_content"]:
        ...
        if len(code_watch["patterns"]) > 3:
            patterns += ", ..."
        lines.append(f"  ✓ Code ({patterns}) - {code_watch['debounce_seconds']}s debounce")
