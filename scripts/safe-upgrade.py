...
Usage:
    python scripts/safe-upgrade.py --source /path/to/agent-os-enhanced --target .agent-os
...
Examples:
    ...
    # Execute upgrade
    python scripts/safe-upgrade.py --source ../agent-os-enhanced --target .agent-os
"""
...
def classify_file(
    ...
    Examples:
        ...
        >>> classify_file("new.md", manifest, Path(".agent-os"), Path("universal"))
        FileState.NEW
    """
...
def print_summary(report: UpgradeReport):
    ...
    if report.backup_path:
        ...
        print(f"\n💡 To rollback:")
        print(f"   rm -rf .agent-os")
        print(f"   mv {report.backup_path} .agent-os")
...
Examples:
  ...
  %(prog)s --source /path/to/agent-os-enhanced --target .agent-os
  ...
  %(prog)s --source /path/to/agent-os-enhanced --yes
    ...
    parser.add_argument(
        "--target",
        default=".agent-os",
        help="Path to local .agent-os directory (default: .agent-os)",
        metavar="DIR"
    )
    ...
    if not source_dir.exists():
        ...
        print(f"\n   Make sure the path points to the agent-os-enhanced repository.", file=sys.stderr)
    ...
    if not manifest_path.exists():
        ...
        print(f"   1. Ensure you're using agent-os-enhanced v1.3.0 or later", file=sys.stderr)
        print(f"   2. Run: cd {args.source} && python scripts/generate-manifest.py --version 1.3.0", file=sys.stderr)
        return 1
    ...
    try:
        ...
        log_message(f"✅ Manifest loaded: {len(manifest['files'])} files tracked", log_file)
