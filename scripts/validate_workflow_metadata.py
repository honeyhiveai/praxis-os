...
def validate_workflow_metadata(workflow_path: Path) -> Tuple[bool, List[str], List[str]]:
    """
    ...
    if "description" in metadata:
        ...
        if len(desc) < 20:
            warnings.append("description is too short (should be detailed and searchable)")
        if not any(char.isspace() for char in desc):
            warnings.append("description should contain multiple words for searchability")
    
    if "estimated_duration" in metadata:
        duration = metadata["estimated_duration"]
        if not any(unit in str(duration).lower() for unit in ["minute", "hour", "day", "week"]):
            errors.append(
                f"estimated_duration should include units: '{duration}'"
            )
    ...
    for i, phase in enumerate(phases):
        ...
        if "key_deliverables" in phase:
            ...
            if not isinstance(deliverables, list) or len(deliverables) == 0:
                errors.append(f"Phase {phase_num}: key_deliverables must be non-empty array")
        
        if "validation_criteria" in phase:
            criteria = phase["validation_criteria"]
            if not isinstance(criteria, list) or len(criteria) == 0:
                errors.append(f"Phase {phase_num}: validation_criteria must be non-empty array")
    
    is_valid = len(errors) == 0
    return is_valid, errors, warnings


def print_results(workflow_name: str, is_valid: bool, errors: List[str], warnings: List[str]) -> None:
...
def main():
    ...
    if len(sys.argv) != 2:
        ...
        print("Example: python scripts/validate_workflow_metadata.py universal/workflows/test_generation_v3")
        sys.exit(2)
