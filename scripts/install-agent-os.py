#!/usr/bin/env python3
"""
Agent OS Enhanced - Fast Installation Script
...
Usage:
    python install-agent-os.py [target_directory]
...
# Configuration
REPO_URL = "https://github.com/honeyhiveai/agent-os-enhanced.git"
MIN_PYTHON = (3, 9)
...
def main():
    ...
    print("=" * 60)
    print("Agent OS Enhanced Installer v1.0.0")
    print("=" * 60)
    print()
...
def clone_repository() -> Path:
    ...
    """
    temp_dir = Path(tempfile.mkdtemp(prefix="agent-os-install-"))
...
def configure_gitignore(target: Path):
    """
    Configure .gitignore to prevent committing ephemeral Agent OS files.
    
    Appends Agent OS patterns to existing .gitignore (or creates new file).
    ...
    # Append Agent OS patterns
    with open(gitignore_path, 'a') as f:
        for pattern in agent_os_patterns:
            f.write(pattern + '\n')
