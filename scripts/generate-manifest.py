...
def validate_manifest(manifest: Dict[str, Any]) -> bool:
    ...
    for rel_path, metadata in manifest["files"].items():
        ...
        if not isinstance(checksum, str) or not checksum.startswith("sha256:"):
            raise ValueError(f"File '{rel_path}' has invalid checksum format: {checksum}")
        
        # Validate checksum length (sha256: + 64 hex chars = 71 total)
        if len(checksum) != 71:
            raise ValueError(f"File '{rel_path}' has invalid checksum length: {len(checksum)}")
        ...
        if not isinstance(last_updated, str) or len(last_updated) != 10:
            raise ValueError(f"File '{rel_path}' has invalid date format: {last_updated}")
...
Examples:
  ...
  %(prog)s --version 1.3.0 --universal-dir /path/to/universal
    ...
    # Validate paths
    if not universal_dir.exists():
        print(f"❌ ERROR: Universal directory not found: {universal_dir}", file=sys.stderr)
        print(f"\n   Make sure you're running from the agent-os-enhanced root directory.", file=sys.stderr)
        return 1
    
    if not universal_dir.is_dir():
        print(f"❌ ERROR: Universal path is not a directory: {universal_dir}", file=sys.stderr)
        return 1
